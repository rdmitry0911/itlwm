...
/*
 * Bounded host-side APSTA owner skeleton.
 *
 * The recovered Apple AppleBCMWLANIO80211APSTAInterface owns the role-7
 * virtual interface lifetime, the AP/SoftAP private state block, the
 * SoftAP parameters, the station table at private +0xb8 (5 entries x
 * 0x30 stride), the AP stats and monitor timers, the AP datapath queues
 * and packet pools, and the async completion cookies tied to the lower
 * AP firmware/Core operations.
 *
 * The local Intel iwx/iwm HALs do not expose an AP/GO firmware MAC
 * context, beacon/probe-response template upload, AP station
 * add/remove, AP key install, CSA/beacon update, or AP firmware event
 * conversion. The local OpenBSD net80211 build keeps IEEE80211_STA_ONLY
 * in scope and the iwx/iwm HOSTAP preflight panics remain in place.
 *
 * While those lower-backend surfaces are absent, this skeleton fails
 * closed before any AP-up transition, with no role-7 success and no
 * station-table publication. It defines the owner type, the role-7
 * carrier validation, the fail-closed lower-backend gate, the
 * state/table storage ownership, and the deterministic cleanup
 * ordering required by the recovered Apple owner contract.
 *
 * Object lifetime is owned by the caller (the setVIRTUAL_IF_CREATE
 * handler) on the call stack: the owner is constructed, validated,
 * cleared, and destroyed inside one handler invocation. The skeleton
 * does not implement an existing-owner check, a duplicate-create
 * already-exists return, a persistent role-7 lifetime, a delete path,
 * an IOService registration, an IO80211 interface, SAP slots, or any
 * datapath surface; promoting the owner to a controller-side instance
 * with create/delete edges depends on the lower-backend support listed
 * above.
 */
class AirportItlwmAPSTAInterface {
public:
    /*
     * Validate the recovered role-7 carrier and initialise driver-
     * private state. Accepts only role 7 with non-NULL MAC and
     * bsd_name pointers; any other role or a NULL pointer is a
     * producer-side input error and leaves the owner inactive.
     */
    bool initWithCarrier(uint32_t role, const uint8_t *mac,
                         const char *bsd_name) {
        clear();
        if (mac == nullptr || bsd_name == nullptr)
            return false;
        if (role != kAirportItlwmAPSTARole)
            return false;
        fActive = true;
        fRole = role;
        memcpy(fMacAddr, mac, sizeof(fMacAddr));
        size_t n = 0;
        while (n < sizeof(fBsdName) - 1 && bsd_name[n] != '\0') {
            fBsdName[n] = bsd_name[n];
            n++;
        }
        fBsdName[n] = '\0';
        fApUpState = 0;
    ...
    /*
     * Lower-backend readiness gate. Returns false until the AP/GO
     * firmware MAC context, beacon/probe-response template upload, AP
     * station add/remove, AP key install, CSA/beacon update, AP
     * firmware event conversion, station-event producer bridge, and
     * removal of the IEEE80211_STA_ONLY build flag are all in place.
     * Until then, the owner cannot transition to AP-up and role-7
     * acquisition cannot return success.
     *
     * Static so the gate can be evaluated before object allocation.
     */
    static bool isLowerBackendReady() {
        return false;
    }

    /*
     * Reset to inactive state. Called both by initWithCarrier() at
     * entry and by the caller during teardown so the cleanup order is
     * deterministic regardless of how role-7 acquisition fails.
     */
    void clear() {
        fActive = false;
        fRole = 0;
        memset(fMacAddr, 0, sizeof(fMacAddr));
        memset(fBsdName, 0, sizeof(fBsdName));
        memset(fStationTable, 0, sizeof(fStationTable));
        fApUpState = 0;
    }

    bool getActive() const { return fActive; }
    uint32_t getRole() const { return fRole; }
    const uint8_t *getMacAddr() const { return fMacAddr; }
    const char *getBsdName() const { return fBsdName; }
    uint8_t getApUpState() const { return fApUpState; }

private:
    bool fActive = false;
    uint32_t fRole = 0;
    uint8_t fMacAddr[6] = {0, 0, 0, 0, 0, 0};
    char fBsdName[kAirportItlwmAPSTAVirtualIfBsdNameSize] = {0};
    /*
     * Station-table storage owned by the APSTA owner per the recovered
     * AppleBCMWLAN private state at +0xb8 (5 entries, 0x30 stride).
     * The skeleton owns the storage; mutation requires the lower AP
     * event producer bridge, which the skeleton does not provide.
     */
    uint8_t fStationTable[kAirportItlwmAPSTAStationTableEntryCount *
                          kAirportItlwmAPSTAStationTableEntryStride] = {0};
    /*
     * Mirror of the recovered Apple AP-up flag at private state
     * +0x26c. Always 0 in this skeleton: the owner never reaches the
     * recovered HostAP success tail because the lower-backend gate is
     * structurally false. The flag may be set only after the lower
     * AP/GO bring-up returns success.
     */
    uint8_t fApUpState = 0;
};
