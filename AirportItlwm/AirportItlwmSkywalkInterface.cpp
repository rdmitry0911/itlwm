...
processBSDCommand(ifnet_t interface, UInt cmd, void *data)
{
    ...
    if ((isApple80211GetIoctl(cmd) || isApple80211SetIoctl(cmd)) &&
        ...
        if (isApple80211GetIoctl(cmd) &&
            req->req_type == APPLE80211_IOC_NSS) {
            /*
             * Tahoe accepts a four-byte public result, then owns an internal
             * eight-byte legacy carrier and copies only its trailing dword.
             * The local helper clears and fills the complete eight-byte NSS
             * structure, while this BSD callback has marshalled only the
             * outer request. Leave the nested caller buffer with
             * IO80211Family; the local helper remains available internally.
             */
            return super::processBSDCommand(interface, cmd, data);
        }
        if (isApple80211GetIoctl(cmd) &&
        ...
        if (isApple80211GetIoctl(cmd) &&
            req->req_type == APPLE80211_IOC_MCS_INDEX_SET) {
            /*
             * Tahoe validates the complete 0x10 public carrier, gives that
             * exact caller buffer to WCL, then falls back through the public
             * NoneProtocol owner. The local helper serializes the same full
             * carrier directly, but this BSD callback has marshalled only the
             * outer request. Leave carrier admission and WCL ownership with
...
processApple80211Ioctl(UInt cmd, apple80211req *req)
{
    ...
    switch (req->req_type) {
        ...
        case APPLE80211_IOC_COUNTERMEASURES:
             ...
             * 0xe082280e leaves. No local COUNTERMEASURES carrier contract
             * is inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_FRAG_THRESHOLD:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. The historical FRAG_THRESHOLD declaration
             * does not establish a public carrier contract.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
            return kIOReturnUnsupported;
        case APPLE80211_IOC_SHORT_SLOT:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. The historical SHORT_SLOT declaration does
             * not establish a public carrier contract.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
            return kIOReturnUnsupported;
        case APPLE80211_IOC_MULTICAST_RATE:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local MULTICAST_RATE carrier contract is
             * inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_SHORT_RETRY_LIMIT:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local SHORT_RETRY_LIMIT carrier contract
             * is inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_LONG_RETRY_LIMIT:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local LONG_RETRY_LIMIT carrier contract
             * is inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_RX_ANTENNA:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local RX_ANTENNA carrier contract is
             * inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_ROM:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local ROM carrier contract is inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
            return kIOReturnUnsupported;
        ...
        case APPLE80211_IOC_BACKGROUND_SCAN:
            /*
             * The current public GET wrapper is an unread fixed 0xe082280e
             * leaf. No local BACKGROUND_SCAN carrier contract is inferred.
             */
            if (cmd == SIOCGA80211)
                return static_cast<IOReturn>(0xe082280e);
            return kIOReturnUnsupported;
        case APPLE80211_IOC_DEBUG_FLAGS:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local DEBUG_FLAGS carrier contract is
             * inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_LAST_RX_PKT_DATA:
            /*
             * The current public GET and SET wrappers are unread fixed
             * 0xe082280e leaves. No local LAST_RX_PKT_DATA carrier contract
             * is inferred.
             */
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_RADIO_INFO:
             ...
             * 0xe082280e leaf. Keep the separate legacy V1 radio route
        ...
        case APPLE80211_IOC_MIMO_POWERSAVE:
            /*
             * The current public GET wrapper is an unread fixed 0xe082280e
             * leaf. No local MIMO_POWERSAVE carrier contract is inferred.
             */
            if (cmd == SIOCGA80211)
                return static_cast<IOReturn>(0xe082280e);
            return kIOReturnUnsupported;
        ...
        case APPLE80211_IOC_AWDL_BT_COEX_AGREEMENT_ENABLED:
            /*
             * Both current 25C56 public directions for these AWDL Bluetooth
             * coexistence selectors are unread fixed 0xe082280e leaves.  Do
             * not manufacture coex agreement, enablement, or protected-period
             * state on this normal non-null Tahoe BSD surface.
             */
#if __IO80211_TARGET >= __MAC_26_0
            if (cmd == SIOCGA80211 || cmd == SIOCSA80211)
                return static_cast<IOReturn>(0xe082280e);
        ...
        case APPLE80211_IOC_VIRTUAL_IF_DELETE:
            ...
            /*
             * Current public Tahoe GET is an unread fixed 0xe082280e leaf.
             * The separately recovered Tahoe SET fixed stub remains below.
             */
            if (cmd == SIOCGA80211)
                return static_cast<IOReturn>(0xe082280e);
#endif // __IO80211_TARGET >= __MAC_26_0
...
setWCL_ASSOCIATEImpl(apple80211AssocCandidates *candidates)
{
    ...
    uint16_t ap_mode = 0;
    uint32_t auth_lower = 0;
    uint32_t auth_upper = 0;
    uint32_t auth_flags = 0;
    memcpy(&ap_mode, raw + TahoeAssociationContracts::kApModeOffset,
           sizeof(ap_mode));
    memcpy(&auth_lower, raw + TahoeAssociationContracts::kAuthLowerOffset,
           sizeof(auth_lower));
    memcpy(&auth_upper, raw + TahoeAssociationContracts::kAuthUpperOffset,
           sizeof(auth_upper));
    memcpy(&auth_flags, raw + TahoeAssociationContracts::kAuthFlagsOffset,
           sizeof(auth_flags));
    uint32_t raw_ssid_len = 0;
    memcpy(&raw_ssid_len,
           raw + TahoeAssociationContracts::kSsidLengthOffset,
           sizeof(raw_ssid_len));
    uint32_t ssid_len = raw_ssid_len;
    const uint8_t *ssid = raw + TahoeAssociationContracts::kSsidOffset;
    uint32_t wcl_key_len = 0;
    uint32_t wcl_key_cipher = 0;
    memcpy(&wcl_key_len,
           raw + TahoeAssociationContracts::kWclKeyLengthOffset,
           sizeof(wcl_key_len));
    memcpy(&wcl_key_cipher,
           raw + TahoeAssociationContracts::kWclKeyCipherTypeOffset,
           sizeof(wcl_key_cipher));
    ...
    */
    if (directSaeWclPassword) {
         ...
         * missing candidate, non-password carrier, or a malformed first
        ...
        /* Keep the existing fault-injection gate meaningful for the separate
         * lab ingress too.  It runs before a password byte is read or a
