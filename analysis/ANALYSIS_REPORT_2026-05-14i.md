...
1. Rebuild both variants on the canonical guest worktree
   ...
   - Default:
     `timeout 420s ssh -i ~/.ssh/automation_itlwm_devops -p 3322
...
3. Reboot the guest with
   `timeout 120s ssh -tt -i ~/.ssh/automation_itlwm_devops -p 3322
...
7. Client-mode regression coverage:
   - Fast diagnostic loop: start
     `<project-root>/start-fast_lab_ap-ap.sh` on
     ...
     `<project-root>/stop-fast_lab_ap-ap.sh`.
