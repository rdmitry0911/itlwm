...
static void stop_case(bool old, int read, int pending, uint16_t winstart,
                      uint16_t winend, bool lockFailure = false) {
    ItlIwn hal;
    auto &sc = hal.com;
    ...
    iwn_node node;
    node.ni.ni_tx_ba[0].ba_winstart = winstart;
    node.ni.ni_tx_ba[0].ba_winend = winend;
    const int qid = sc.first_agg_txq;
    sc.agg_queue_mask = 1 << qid;
    sc.sc_tx_ba[0].wn = &node;
    ...
    ItlIwn::iwn_ampdu_tx_stop(&sc.sc_ic, &node.ni, 0);
    if (lockFailure) {
        assert(ring.queued == pending && ring.read == read);
        assert(sc.freed == 0 && sc.sc_tx_ba[0].wn == &node);
        assert(!sc.stopped[qid]);
        sc.failLock = false;
        ItlIwn::iwn_ampdu_tx_stop(&sc.sc_ic, &node.ni, 0);
    }
    assert(ring.queued == 0);
    assert(ring.cur == (winstart & 255) && ring.read == ring.cur);
    assert(sc.freed == pending && sc.nodeReleases == pending && sc.resets == pending);
    assert(sc.agg_queue_mask == 0 && sc.sc_tx_ba[0].wn == nullptr);
    assert(node.ni.ni_tx_ba[0].ba_bitmap == 0 && (node.disable_tid & 1));
    assert(!sc.locked);
...
int main() {
    for (bool old : {false, true}) {
        ...
        stop_case(old, 29, 1, 29, 92);
        stop_case(old, 250, 8, 250, 313);
        stop_case(old, 255, 1, 4095, 62);
        stop_case(old, 17, 0, 17, 80);
        stop_case(old, 12, 3, 12, 14); // logical end is not exclusive cur
        stop_case(old, 29, 1, 29, 92, true);
    }
    ItlIwn hal;
    iwn_tx_ring ring;
    ...
    std::puts("PASS: actual IWN/4965 STA BA stop deactivates, drains owned descriptors, then rebases");
}
