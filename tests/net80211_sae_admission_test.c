...
assert_rejected(struct ieee80211_pae_selected_bss selected)
{
	...
	assert(ieee80211_sae_admission_group19(&selected, &admission) == 0);
...
main(void)
{
	...
	assert(ieee80211_sae_admission_group19(&selected, &admission) == 1);
	...
	assert(ieee80211_sae_admission_group19(&selected, &admission) == 1);
	assert(admission.group == IEEE80211_SAE_ADMISSION_GROUP_19);
	assert(admission.method == IEEE80211_SAE_ADMISSION_METHOD_H2E);
	assert(admission.rsnxe_capabilities ==
	    IEEE80211_SAE_ADMISSION_RSNXE_H2E);
	...
	selected = valid_selected();
	selected.sae_scan_flags |= IEEE80211_SAE_SCAN_H2E_ONLY_SELECTOR;
	...
	for (uint32_t flag = 1; flag != 0; flag <<= 1) {
		if ((flag & IEEE80211_SAE_ADMISSION_GROUP19_ALLOWED_FLAGS) != 0)
			continue;
		selected = valid_selected();
