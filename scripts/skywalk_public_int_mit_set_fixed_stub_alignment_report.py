#!/usr/bin/env python3
"""Generate and verify Skywalk public INT_MIT SET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_int_mit_set_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-535-skywalk-public-int-mit-set-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-int-mit-set-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-int-mit-set-fixed-stub-alignment-v1",
        "source_base_revision": "d7fd2c979abbb502a5e335ac6048d7a0a84de7b8",
        "reference": {
            "bootkc_sha256": "eb5691e94b750df8316f8474245966e02d1badd696f78aa27f003766c9bff06d",
            "bootkc_uuid": "F0ACEF59-61D0-DEDC-C1D2-BECE30DD94E5",
            "embedded_kext_uuid": "8FB4B7F0-D656-3539-B8D6-C1327A50377C",
            "nlist_index": 7116,
            "public_wrapper": "0xffffff80021c37db",
            "next_symbol": "0xffffff80021c37e6",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_set_fixed_stub": all(
                ...
                for token in (
                    ...
                    "symbol=__Z20apple80211setINT_MITP23IO80211SkywalkInterfacePv",
                    "nlist_index=7116",
                    "n_type=0x0f",
                    "n_sect=1",
                    "n_desc=0x0000",
                    "symbol_vmaddr=0xffffff80021c37db",
                    "symbol_vmaddr_end=0xffffff80021c37e6",
                    "symbol_fileoff=0x20c37db",
                    "symbol_fileoff_end=0x20c37e6",
                    "symbol_bytes=0x0b",
                    "next_nlist_index=7060",
                    "__Z18apple80211setPOWERP23IO80211SkywalkInterfaceP21apple80211_power_data",
                    "next_symbol_vmaddr=0xffffff80021c37e6",
                    "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
                    "0xe082280e",
            ...
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "0xffffff80021c37db",
                    "0xe082280e",
                    "compile-time Tahoe-only SET branch",
                    "normal non-null Tahoe BSD SET",
                    "card-specific route remains excluded",
                    "does not claim outer-null, carrier/ABI, GET beyond its previously aligned leaf, V1/helper, Virtual IOCTL, card-specific, association, firmware, runtime-execution, or broader Tahoe behavior parity",
                    "No private carrier or selector is constructed or invoked",
                )
            ),
            "normal_nonnull_tahoe_public_set_returns_exact_unread_status": (
                dispatcher.count("case APPLE80211_IOC_INT_MIT:") == 1
                and "if (cmd == SIOCSA80211)" in fixed_case
                and "return static_cast<IOReturn>(0xe082280e);" in fixed_case
                and "req->req_data" not in fixed_case
                and "getINT_MIT" not in fixed_case
                and "return kIOReturnSuccess;" not in fixed_case
            ),
            "previously_aligned_get_leaf_is_preserved": (
                fixed_case.index("if (cmd == SIOCSA80211)")
                < fixed_case.index("if (cmd == SIOCGA80211)")
                and fixed_case.count("return static_cast<IOReturn>(0xe082280e);") == 2
            ),
            "non_get_and_pre26_boundaries_remain_separate": (
                ...
                < dispatcher.index("case APPLE80211_IOC_RSSI:")
                and "case APPLE80211_IOC_INT_MIT:" not in dispatcher[
                    dispatcher.index("#endif // __IO80211_TARGET >= __MAC_26_0", dispatcher.index("case APPLE80211_IOC_INT_MIT:"))
            ...
            "null_fallback_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_INT_MIT:")
            ),
            ...
            "legacy_helper_and_virtual_route_remain_separate": (
                "case APPLE80211_IOC_INT_MIT:  // 18\n            IOCTL_GET(request_type, INT_MIT, apple80211_intmit_data);" in v1
                and "imd->int_mit = APPLE80211_INT_MIT_AUTO;" in legacy_int_mit
                and "return kIOReturnSuccess;" in legacy_int_mit
                and "FUNC_IOCTL_GET(INT_MIT, apple80211_intmit_data)" in header
                and "APPLE80211_IOC_INT_MIT" not in virtual
            ),
            "card_specific_route_remains_excluded_before_dispatch": (
                "kIocIntMit" not in routes
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public INT_MIT SET alignment validation failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public INT_MIT SET alignment validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
