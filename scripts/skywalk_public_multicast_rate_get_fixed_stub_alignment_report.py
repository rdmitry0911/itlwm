#!/usr/bin/env python3
"""Generate and verify Skywalk public MULTICAST_RATE GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_multicast_rate_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-546-skywalk-public-multicast-rate-get-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-multicast-rate-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        ...
        "reference": {
            ...
            "nlist_index": 7507,
            "public_wrapper": "0xffffff80021beac6",
            "next_symbol": "0xffffff80021bead1",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z27apple80211getMULTICAST_RATEP23IO80211SkywalkInterfacePv",
                "nlist_index=7507", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021beac6", "symbol_vmaddr_end=0xffffff80021bead1",
                "symbol_fileoff=0x20beac6", "symbol_fileoff_end=0x20bead1", "symbol_bytes=0x0b",
                "next_nlist_index=7721", "__Z30apple80211getSHORT_RETRY_LIMITP23IO80211SkywalkInterfacePv",
                "next_symbol_vmaddr=0xffffff80021bead1", "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
            ...
            "outer_null_and_bsd_boundaries_remain_explicit": (
                ...
                and dispatcher.index("if (req == NULL)\n        return kIOReturnUnsupported;") < dispatcher.index("case APPLE80211_IOC_MULTICAST_RATE:")
            ...
            "legacy_virtual_and_card_boundaries_remain_separate": (
                "APPLE80211_IOC_MULTICAST_RATE" not in v1
                and "APPLE80211_IOC_MULTICAST_RATE" not in virtual
                and "kIocMulticastRate" not in routes
                and "APPLE80211_IOC_MULTICAST_RATE" not in routes
                and "default:\n            return false;" in routes
                and "routeTahoeSkywalkIoctl(interface, &req," in card
            ),
            "selector_is_declared_without_local_carrier_access": (
                "#define APPLE80211_IOC_MULTICAST_RATE            34" in ioctl
                and "req->req_data" not in multicast
            ),
            "all_active_skywalk_source_phases_and_target_guard_are_explicit": (
                all(marker in project for marker in markers)
                and "case APPLE80211_IOC_MULTICAST_RATE:" in tahoe_block
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public MULTICAST_RATE GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public MULTICAST_RATE GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
