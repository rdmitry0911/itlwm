#!/usr/bin/env python3
"""Generate and verify legacy V1 BTCOEX_PROFILES non-null fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/legacy_btcoex_profiles_non_null_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-521-legacy-btcoex-profiles-non-null-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/legacy-btcoex-profiles-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    teardown = section(main, "if (btcProfile != NULL)", "super::free();")
    tahoe_route = section(
        skywalk,
        "case APPLE80211_IOC_BTCOEX_PROFILES:",
    ...
    return {
        "schema": "itlwm-legacy-btcoex-profiles-non-null-fixed-stub-alignment-v1",
        "source_base_revision": "c985a2a3284843893c192e34170962f28142481d",
        "reference": {
            ...
            "public_wrapper": "0xffffff80021c502e",
            "next_symbol": "0xffffff80021c5039",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_fixed_stub_and_null_boundary": all(
                ...
                for token in (
                    ...
                    "n_sect=1",
                    "n_desc=0x0000",
                    "__Z28apple80211setBTCOEX_PROFILESP23IO80211SkywalkInterfacePv",
                    "symbol_vmaddr=0xffffff80021c502e",
                    "symbol_vmaddr_end=0xffffff80021c5039",
                    "symbol_fileoff=0x20c502e",
                    "symbol_fileoff_end=0x20c5039",
                    "symbol_bytes=0x0b",
                    "next_nlist_index=7461",
                    "__Z26apple80211setBTCOEX_CONFIGP23IO80211SkywalkInterfacePv",
                    "next_symbol_vmaddr=0xffffff80021c5039",
            ...
            "legacy_selector_carrier_route_and_null_guard_remain": (
                "#define APPLE80211_IOC_BTCOEX_PROFILES 221" in ioctl
                and "uint32_t    version;" in carrier
                and "uint32_t    profile_cnt;" in carrier
                and "profiles[141][4];" in carrier
                and "__attribute__((packed))" in carrier
                and "FUNC_IOCTL(BTCOEX_PROFILES, apple80211_btc_profiles_data)" in hpp
                and "if (request_type != SIOCGA80211 && request_type != SIOCSA80211)" in cpp
                and "IOCTL(request_type, BTCOEX_PROFILES, apple80211_btc_profiles_data);" in legacy_dispatch
            ...
            "non_null_setter_matches_fixed_status_without_carrier_read_or_cache_write": (
                ...
                and "btcProfile" not in legacy_setter
                and "IOFree" not in legacy_setter
                and "IOMalloc" not in legacy_setter
                and "memcpy" not in legacy_setter
            ),
            "legacy_getter_and_teardown_pointer_are_separate": (
                "if (!data || !btcProfile)" in legacy_getter
                and "memcpy(data, btcProfile, sizeof(struct apple80211_btc_profiles_data));" in legacy_getter
                and "return kIOReturnSuccess;" in legacy_getter
                and "struct apple80211_btc_profiles_data *btcProfile;" in hpp
                and "IOFree(btcProfile, sizeof(struct apple80211_btc_profiles_data));" in teardown
                and "btcProfile = NULL;" in teardown
            ),
            "tahoe_direct_gate_remains_separate": (
                "kApple80211ClassOwnerAbsent =" in skywalk
                and "static_cast<IOReturn>(0xe082280e)" in skywalk
                and "case APPLE80211_IOC_BTCOEX_PROFILES:" in tahoe_route
...
def main():
    ...
    if failed:
        raise ValueError("legacy BTCOEX_PROFILES fixed-stub checks failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"legacy BTCOEX_PROFILES fixed-stub validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
