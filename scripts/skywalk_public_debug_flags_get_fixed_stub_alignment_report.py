#!/usr/bin/env python3
"""Generate and verify Skywalk public DEBUG_FLAGS GET fixed-stub evidence."""
...
def report():
    ...
    return {
        ...
        "reference": {
            ...
            "nlist_index": 7290,
            "public_wrapper": "0xffffff80021bee4c",
            "next_symbol": "0xffffff80021bee57",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z24apple80211getDEBUG_FLAGSP23IO80211SkywalkInterfacePv",
                "nlist_index=7290", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021bee4c", "symbol_vmaddr_end=0xffffff80021bee57",
                "symbol_fileoff=0x20bee4c", "symbol_fileoff_end=0x20bee57", "symbol_bytes=0x0b",
                "next_nlist_index=7656", "__Z29apple80211getLAST_RX_PKT_DATAP23IO80211SkywalkInterfacePv",
                "next_symbol_vmaddr=0xffffff80021bee57",
                "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public DEBUG_FLAGS GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public DEBUG_FLAGS GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
