#!/usr/bin/env python3
"""Generate and verify public AWDL tree/guard fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_tree_guard_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-576-skywalk-public-awdl-tree-guard-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-tree-guard-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_MAX_TREE_DEPTH", 123, 7843, "0xffffff80021bf613", 7870, "0xffffff80021c444a"),
    ("AWDL_GUARD_TIME", 124, 7567, "0xffffff80021bf61e", 7604, "0xffffff80021c4455"),
)
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-tree-guard-fixed-stubs-alignment-v1",
        "source_base_revision": "6c136f5c91ae504e43c9721edbd95bcced781b85",
        "reference": {
        ...
        "checks": {
            ...
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL tree topology or guard-time state",
                    "No AWDL_ELECTION_ID, AWDL_BSSID, AWDL SYNC_PARAMS, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            ...
            "null_fallback_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_AWDL_MAX_TREE_DEPTH:")
            ),
            ...
            "no_card_specific_awdl_tree_or_guard_route_is_introduced": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL tree/guard alignment checks failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL tree/guard alignment validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
