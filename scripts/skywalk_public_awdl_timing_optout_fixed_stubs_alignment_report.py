#!/usr/bin/env python3
"""Generate and verify public AWDL timing/opt-out fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_timing_optout_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-579-skywalk-public-awdl-timing-optout-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-timing-optout-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_PERIODIC_SYNC_FRAME_PACKET_LIFETIME", 138, 8249, "0xffffff80021bf8d9", 8250, "0xffffff80021c46f5"),
    ("AWDL_MASTER_MODE_SYNC_FRAME_PERIOD", 139, 8226, "0xffffff80021bf8e4", 8228, "0xffffff80021c4700"),
    ("AWDL_NON_ELECTION_MASTER_MODE_SYNC_FRAME_PERIOD", 140, 8252, "0xffffff80021bf8ef", 8253, "0xffffff80021c470b"),
    ("AWDL_EXPLICIT_AVAILABILITY_WINDOW_EXTENSION_OPT_OUT", 141, 8254, "0xffffff80021bf8fa", 8255, "0xffffff80021c4716"),
)
...
def report():
    ...
    awdl_route = section(dispatcher, "case APPLE80211_IOC_AWDL_PERIODIC_SYNC_FRAME_PACKET_LIFETIME:", "case APPLE80211_IOC_CIPHER_KEY:")
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-timing-optout-fixed-stubs-alignment-v1",
        "source_base_revision": "371764dc4f0aaea119a93ebe46124924e52d0a3a",
        "reference": {
        ...
        "checks": {
            ...
            "reference_raw_records_all_eight_exact_fixed_leaves": (
            ...
            "note_records_scope_and_nonclaims": all(
                token in note for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL frame-period, lifetime, or availability-window opt-out state",
                    "No AWDL_GET_AWDL_MASTER_DATABASE, AWDL_CIPHER_KEY, AWDL synchronization-channel, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            "normal_nonnull_tahoe_public_get_and_set_return_exact_fixed_status_unread": (
                all(marker in awdl_route for marker in case_markers)
            ...
            "null_fallback_remains_outside_this_layer": null_guard in dispatcher and dispatcher.index(null_guard) < dispatcher.index("case APPLE80211_IOC_AWDL_PERIODIC_SYNC_FRAME_PACKET_LIFETIME:"),
            ...
            "no_card_specific_awdl_timing_or_optout_route_is_introduced": all(name not in routes for name, *_ in SURFACES),
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public AWDL timing/opt-out alignment checks failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public AWDL timing/opt-out validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
