...
STEP_ID = "step:itlwm-rm-05a0b0-auth-mgmt-tx-enqueue-drain-boundary"
ROADMAP_ITEM_ID = "itlwm-rm-05a0b0-auth-mgmt-tx-enqueue-drain-boundary"
INPUT_HEAD = ""
DEFAULT_OUTPUT = Path("evidence/runtime/tahoe_auth_mgmt_tx_enqueue_drain_boundary.json")
AUTH_SUBTYPE = "0xb0"
IWN_FILTER_BSS = 0x20
...
REQUIRED_FIELDS = [
    ...
    "auth.tx_completion_not_claimed",
...
EQUALS = {
    ...
    "auth.tx_completion_not_claimed": True,
...
def build_evidence(args):
    ...
    nonce = "itlwm-auth-mgmt-tx-" + uuid.uuid4().hex[:16]
    ...
    log_result = base.run_guest_script(
        args,
        "guest unified log capture for AUTH management TX enqueue/drain markers",
        build_auth_log_script(nonce, args.log_last),
        args.log_timeout,
    )
    ioreg_result = base.run_guest_script(
        args,
        "guest IORegistry capture for AUTH management TX enqueue/drain properties",
        build_ioreg_script(),
        args.command_timeout,
    )
    ...
    evidence = {
        "schema_version": "itlwm-tahoe-auth-mgmt-tx-enqueue-drain-boundary/v1",
        "step_id": STEP_ID,
        ...
        "candidate_fix": {
            ...
            "source_delta_paths": [
                ...
                "scripts/capture_tahoe_auth_mgmt_tx_enqueue_drain.py",
                "evidence/runtime/tahoe_auth_mgmt_tx_enqueue_drain_boundary.json",
            ],
        },
        "non_claims": {
        ...
        "verdict": {
            ...
            "reason": "live Tahoe guest proved AUTH(seq=1) enqueue, management-queue drain, iwn TX ring entry, PMK delivery, and BSS-filtered AUTH response-window setup"
            if ready
            else "one or more live Tahoe AUTH management enqueue/drain predicates were not satisfied",
        },
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Capture sanitized live Tahoe evidence for AUTH management TX enqueue/drain."
    )
