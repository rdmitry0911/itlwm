...
raw="$root/docs/reference/artifacts/skywalk-rand-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "RAND SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z17apple80211setRANDP23IO80211SkywalkInterfacePv' \
    'nlist_index=7028' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3b19' \
    'symbol_vmaddr_end=0xffffff80021c3b24' \
    'symbol_fileoff=0x20c3b19' \
    'symbol_fileoff_end=0x20c3b24' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7079' \
    'next_symbol=__Z19apple80211setRSN_IEP23IO80211SkywalkInterfaceP22apple80211_rsn_ie_data' \
    'next_symbol_vmaddr=0xffffff80021c3b24' \
    'next_symbol_fileoff=0x20c3b24' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_RAND[[:space:]]+45([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 45'
...
')
rand_case_line=$(line_of 'case APPLE80211_IOC_RAND:')
[ -n "$outer_null_line" ] && [ -n "$rand_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$rand_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_RAND:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_RAND:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
rand=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_RAND:/,/case APPLE80211_IOC_BACKGROUND_SCAN:/p')
printf '%s\n' "$rand" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$rand" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$rand" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$rand" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$rand" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)RAND[[:space:]]*\(' "$source"; then
    fail 'a local RAND producer was introduced'
fi
if grep -Eq 'APPLE80211_IOC_RAND|kIocRand|case[[:space:]]+45' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_rand_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe RAND SET fixed-stub contract: PASS\n'
