#!/usr/bin/env python3
"""Generate and verify legacy V1 BTCOEX_CONFIG non-null fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/legacy_btcoex_config_non_null_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-519-legacy-btcoex-config-non-null-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/legacy-btcoex-config-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        "schema": "itlwm-legacy-btcoex-config-non-null-fixed-stub-alignment-v1",
        "source_base_revision": "2d0e300f892cd58a925db63f6b5aa5f3d18d0cf7",
        "reference": {
            ...
            "public_wrapper": "0xffffff80021c5039",
            "next_symbol": "0xffffff80021c5044",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_fixed_stub_and_null_boundary": all(
                ...
                for token in (
                    ...
                    "symbol_vmaddr=0xffffff80021c5039",
                    "symbol_vmaddr_end=0xffffff80021c5044",
                    "symbol_fileoff=0x20c5039",
                    "symbol_fileoff_end=0x20c5044",
                    "symbol_bytes=0x0b",
                    "next_nlist_index=7606",
                    "__Z28apple80211setAWDL_STATISTICSP23IO80211SkywalkInterfacePv",
                    "next_symbol_vmaddr=0xffffff80021c5044",
            ...
            "legacy_selector_carrier_route_and_null_guard_remain": (
                "#define APPLE80211_IOC_BTCOEX_CONFIG 222" in ioctl
                and "uint32_t version;" in carrier
                and "uint32_t enable_2G;" in carrier
                and "uint32_t profile_2g;" in carrier
                and "uint32_t enable_5G;" in carrier
                and "uint32_t profile_5G;" in carrier
                and "__attribute__((packed))" in carrier
                and "FUNC_IOCTL(BTCOEX_CONFIG, apple80211_btc_config_data)" in hpp
                and "if (request_type != SIOCGA80211 && request_type != SIOCSA80211)" in cpp
                and "IOCTL(request_type, BTCOEX_CONFIG, apple80211_btc_config_data);" in legacy_dispatch
            ...
            "non_null_setter_matches_fixed_status_without_carrier_read_or_cache_write": (
                ...
                and "btcConfig" not in legacy_setter
            ...
            "legacy_getter_and_remaining_readback_field_are_separate": (
                ...
                and "memcpy(data, &btcConfig, sizeof(struct apple80211_btc_config_data));" in legacy_getter
                and "return kIOReturnSuccess;" in legacy_getter
                and "struct apple80211_btc_config_data btcConfig;" in hpp
                and cpp.count("btcConfig") == 1
            ),
            "tahoe_direct_gate_remains_separate": (
                "kApple80211ClassOwnerAbsent =" in skywalk
                and "static_cast<IOReturn>(0xe082280e)" in skywalk
                and "case APPLE80211_IOC_BTCOEX_CONFIG:" in tahoe_route
...
def main():
    ...
    if failed:
        raise ValueError("legacy BTCOEX_CONFIG fixed-stub checks failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"legacy BTCOEX_CONFIG fixed-stub validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
