...
raw="$root/docs/reference/artifacts/skywalk-tx-antenna-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "TX_ANTENNA SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z23apple80211setTX_ANTENNAP23IO80211SkywalkInterfacePv' \
    'nlist_index=7279' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3ac1' \
    'symbol_vmaddr_end=0xffffff80021c3acc' \
    'symbol_fileoff=0x20c3ac1' \
    'symbol_fileoff_end=0x20c3acc' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7274' \
    'next_symbol=__Z23apple80211setRX_ANTENNAP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3acc' \
    'next_symbol_fileoff=0x20c3acc' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_TX_ANTENNA[[:space:]]+37([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 37'
...
')
tx_case_line=$(line_of 'case APPLE80211_IOC_TX_ANTENNA:')
[ -n "$outer_null_line" ] && [ -n "$tx_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$tx_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_TX_ANTENNA:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_TX_ANTENNA:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
tx=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_TX_ANTENNA:/,/case APPLE80211_IOC_RX_ANTENNA:/p')
printf '%s\n' "$tx" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$tx" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$tx" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$tx" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$tx" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)TX_ANTENNA[[:space:]]*\(' "$source"; then
    fail 'a local TX_ANTENNA producer was introduced'
fi
grep -Fq 'IOCTL_GET(request_type, TX_ANTENNA, apple80211_antenna_data);' "$v1" ||
    fail 'separate V1 GET route disappeared'
if grep -Fq 'IOCTL_SET(request_type, TX_ANTENNA' "$v1"; then
    fail 'a V1 TX_ANTENNA setter was introduced'
fi
if grep -Eq 'TX_ANTENNA|kIocTxAntenna|case[[:space:]]+37' "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_tx_antenna_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe TX_ANTENNA SET fixed-stub contract: PASS\n'
