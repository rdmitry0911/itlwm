#!/usr/bin/env python3
"""Generate and verify public AWDL RSSI/AES/timing/control stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_rssi_aes_timing_control_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-583-skywalk-public-awdl-rssi-aes-timing-control-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-rssi-aes-timing-control-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_RSSI_MEASUREMENT_REQUEST", 159, 8188, "0xffffff80021bfae8", 8194, "0xffffff80021c49c2"),
    ("AWDL_AES_KEY", 160, 7367, "0xffffff80021bfaf3", 7394, "0xffffff80021c49cd"),
    ("AWDL_SCAN_RESERVED_TIME", 161, 8039, "0xffffff80021bfafe", 8059, "0xffffff80021c49d8"),
    ("AWDL_CTL", 162, 7136, "0xffffff80021bfb09", 7154, "0xffffff80021c49e3"),
    ("AWDL_SOCIAL_TIME_SLOTS", 163, 8010, "0xffffff80021bfb14", 8021, "0xffffff80021c49ee"),
)
...
def report():
    ...
    after_tahoe_branch = awdl_route[
        awdl_route.index("#endif // __IO80211_TARGET >= __MAC_26_0")
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-rssi-aes-timing-control-fixed-stubs-alignment-v1",
        "source_base_revision": "e0d9c1339105bedffee99e54779ee4022ebe67fc",
        "reference": {
        ...
        "checks": {
            ...
            "reference_raw_records_all_ten_exact_fixed_leaves": all(
                token in raw for token in raw_surface_tokens
            )
            and raw.count("get_body_hex=554889e5b80e2882e05dc3") == 5
            and raw.count("set_body_hex=554889e5b80e2882e05dc3") == 5
            and raw.count("get_reads_neither_public_argument=true") == 5
            and raw.count("set_reads_neither_public_argument=true") == 5
            and raw.count("get_n_type=0x0f") == 5
            and raw.count("set_n_type=0x0f") == 5,
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL RSSI-request, AES-key, scan-reservation, control, or social-slot state",
                    "No AWDL_PEER_TRAFFIC_REGISTRATION, AWDL_FORCED_ROAM_CONFIG, AWDL_QUIET, AWDL_OOB_REQUEST, AWDL_SYNC_FRAME_TEMPLATE, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            "normal_nonnull_tahoe_public_get_and_set_return_exact_fixed_status_unread": all(
                marker in awdl_route for marker in case_markers
            )
            ...
            < dispatcher.index("case APPLE80211_IOC_AWDL_RSSI_MEASUREMENT_REQUEST:"),
            ...
            "no_card_specific_awdl_rssi_aes_timing_or_control_route_is_introduced": all(
                name not in routes for name, *_ in SURFACES
            ),
            "only_one_public_dispatch_case_per_awdl_selector": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL RSSI/AES/timing/control alignment checks failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL RSSI/AES/timing/control validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
