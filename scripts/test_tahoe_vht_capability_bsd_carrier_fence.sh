...
grep -Fq 'does not claim they select the same recovered private-table path' "$evidence" || fail 'alias caveat is missing from evidence'
...
[ -n "$fence_line" ] && [ -n "$dispatch_line" ] && [ -n "$tahoe_guard_line" ] && [ -n "$tahoe_endif_line" ] || fail 'BSD fence or dispatcher is missing'
...
fence=$(printf '%s\n' "$bsd_bridge" | sed -n "$((fence_line - 1)),$((fence_line + 20))p")
printf '%s\n' "$fence" | grep -Fq '(isApple80211GetIoctl(cmd) || isApple80211SetIoctl(cmd))' || fail 'BSD VHT fence is not both GET and SET'
