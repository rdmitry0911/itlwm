...
raw="$root/docs/reference/artifacts/skywalk-multicast-rate-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "MULTICAST_RATE SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z27apple80211setMULTICAST_RATEP23IO80211SkywalkInterfacePv' \
    'nlist_index=7544' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3aa0' \
    'symbol_vmaddr_end=0xffffff80021c3aab' \
    'symbol_fileoff=0x20c3aa0' \
    'symbol_fileoff_end=0x20c3aab' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7752' \
    'next_symbol=__Z30apple80211setSHORT_RETRY_LIMITP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3aab' \
    'next_symbol_fileoff=0x20c3aab' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_MULTICAST_RATE[[:space:]]+34([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 34'
...
')
multicast_case_line=$(line_of 'case APPLE80211_IOC_MULTICAST_RATE:')
[ -n "$outer_null_line" ] && [ -n "$multicast_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$multicast_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_MULTICAST_RATE:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_MULTICAST_RATE:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
multicast=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_MULTICAST_RATE:/,/case APPLE80211_IOC_SHORT_RETRY_LIMIT:/p')
printf '%s\n' "$multicast" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$multicast" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$multicast" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$multicast" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$multicast" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)MULTICAST_RATE[[:space:]]*\(' "$source"; then
    fail 'a local MULTICAST_RATE producer was introduced'
fi
if grep -Eq 'MULTICAST_RATE|kIocMulticastRate|case[[:space:]]+34' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_multicast_rate_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe MULTICAST_RATE SET fixed-stub contract: PASS\n'
