#!/usr/bin/env python3
"""Generate and verify Skywalk public RAND GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_rand_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-553-skywalk-public-rand-get-fixed-stub-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-rand-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        ...
        "reference": {
            ...
            "nlist_index": 7017,
            "public_wrapper": "0xffffff80021bec85",
            "next_symbol": "0xffffff80021bec90",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z17apple80211getRANDP23IO80211SkywalkInterfacePv",
                "nlist_index=7017", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021bec85", "symbol_vmaddr_end=0xffffff80021bec90",
                "symbol_fileoff=0x20bec85", "symbol_fileoff_end=0x20bec90", "symbol_bytes=0x0b",
                "next_nlist_index=7073", "__Z19apple80211getRSN_IEP23IO80211SkywalkInterfaceP22apple80211_rsn_ie_data",
                "next_symbol_vmaddr=0xffffff80021bec90",
                "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
                "0xe082280e", "reads neither public argument",
            )),
            "note_records_scope_and_nonclaims": all(token in note for token in (
                "IOC 45", "0xffffff80021bec85", "0xe082280e", "compile-time Tahoe-only case",
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public RAND GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public RAND GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
