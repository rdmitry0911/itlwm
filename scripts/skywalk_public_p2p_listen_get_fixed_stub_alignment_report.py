#!/usr/bin/env python3
"""Generate and verify Skywalk public P2P_LISTEN GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_p2p_listen_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-565-skywalk-public-p2p-listen-get-fixed-stub-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-p2p-listen-get-public-fixed-stub-bootkc-current/raw.txt"
...
MACRO = "APPLE80211_IOC_P2P_LISTEN"
VALUE = 92
...
def report():
    ...
    tahoe_block = section(dispatcher, "#if __IO80211_TARGET >= __MAC_26_0\n        case APPLE80211_IOC_P2P_LISTEN:", "        case APPLE80211_IOC_SOFTAP_STATS:")
    group = section(dispatcher, "case APPLE80211_IOC_P2P_LISTEN:", "        case APPLE80211_IOC_SOFTAP_STATS:")
    ...
    return {
        "schema": "itlwm-skywalk-public-p2p-listen-get-fixed-stub-alignment-v1",
        "source_base_revision": "edda001701da3e78719771fed6ca0fa6caa271ff",
        "reference": {
            ...
            "selector": {"name": "P2P_LISTEN", "ioc": VALUE, "nlist_index": 7241, "public_wrapper": "0xffffff80021bf26f", "fileoff": "0x20bf26f", "symbol": "__Z23apple80211getP2P_LISTENP23IO80211SkywalkInterfacePv", "next_nlist_index": 7143, "next_symbol": "__Z21apple80211getP2P_SCANP23IO80211SkywalkInterfacePv"},
        },
        "scope": {
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "selector=P2P_LISTEN", "nlist_index=7241", "symbol_vmaddr=0xffffff80021bf26f",
                "symbol_fileoff=0x20bf26f", "symbol_bytes=0x0b", "next_nlist_index=7143",
            ...
            "note_records_scope_and_nonclaims": all(token in note for token in (
                "IOC 92", "0xffffff80021bf26f", "0xe082280e", "compile-time Tahoe-only case",
                "Tahoe selector remains absent from the pre-26 Skywalk route",
                "Legacy V1 and Virtual `P2P_LISTEN` SET routes remain separate and unmodified",
                "No local GET carrier contract is inferred",
                "does not claim outer-null dispatch behavior, a carrier contract, P2P-listen behavior, SET behavior, P2P scan behavior, V1, Virtual IOCTL, card-specific behavior, firmware, runtime-execution, radio, association, traffic, or broader Tahoe behavior parity",
            ...
            "legacy_virtual_and_card_set_boundaries_remain_explicit": (
                f"case {MACRO}:" in v1
                and "IOCTL_SET(request_type, P2P_LISTEN, apple80211_p2p_listen_data);" in v1
                and f"case {MACRO}:" in virtual
                and "IOCTL_SET(request_type, P2P_LISTEN, apple80211_p2p_listen_data);" in virtual
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public P2P_LISTEN GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public P2P_LISTEN GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
