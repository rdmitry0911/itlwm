...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-rx-antenna-get-fixed-stub-alignment-v1",
        ...
        "scope": {"public_nonnull_request_object_tahoe_bsd_get_only": True, "carrier_is_not_observed": True, "rx_antenna_set_modified": False, "outer_null_dispatch_modified": False, "pre26_route_modified": False, "card_specific_route_modified": False, "legacy_v1_modified": False, "virtual_ioctl_modified": False, "carrier_abi_added": False, "deployment": False, "radio_or_association": False, "runtime_selector_invocation": False, "traffic": False},
        "checks": {
            ...
            "note_records_scope_and_nonclaims": all(token in note for token in ("IOC 38", "0xffffff80021beaf2", "0xe082280e", "compile-time Tahoe-only case", "selector remains absent from the pre-26 switch", "No local RX_ANTENNA carrier contract is inferred", "card-specific route has no RX_ANTENNA entry", "does not claim outer-null dispatch behavior, a RX_ANTENNA carrier contract, SET behavior, antenna behavior, V1, Virtual IOCTL, card-specific behavior, firmware, runtime-execution, radio, association, traffic, or broader Tahoe behavior parity", "No private carrier or selector is constructed or invoked")),
            "public_tahoe_get_returns_exact_unread_status": dispatcher.count("case APPLE80211_IOC_RX_ANTENNA:") == 1 and "if (cmd == SIOCGA80211)" in rx and "return static_cast<IOReturn>(0xe082280e);" in rx and "req->req_data" not in rx and "return kIOReturnSuccess;" not in rx,
            "tahoe_nonget_and_pre26_boundaries_remain_explicit": "return kIOReturnUnsupported;" in rx and "SIOCSA80211" not in rx and "#endif // __IO80211_TARGET >= __MAC_26_0" in rx and "case APPLE80211_IOC_RX_ANTENNA:" not in pre26_dispatcher,
