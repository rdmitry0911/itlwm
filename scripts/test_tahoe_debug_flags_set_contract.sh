...
raw="$root/docs/reference/artifacts/skywalk-debug-flags-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "DEBUG_FLAGS SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z24apple80211setDEBUG_FLAGSP23IO80211SkywalkInterfacePv' \
    'nlist_index=7325' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3c53' \
    'symbol_vmaddr_end=0xffffff80021c3c5e' \
    'symbol_fileoff=0x20c3c53' \
    'symbol_fileoff_end=0x20c3c5e' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7684' \
    'next_symbol=__Z29apple80211setLAST_RX_PKT_DATAP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3c5e' \
    'next_symbol_fileoff=0x20c3c5e' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_DEBUG_FLAGS[[:space:]]+52([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 52'
...
')
debug_case_line=$(line_of 'case APPLE80211_IOC_DEBUG_FLAGS:')
[ -n "$carrier_null_line" ] && [ -n "$debug_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$carrier_null_line" -lt "$debug_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_DEBUG_FLAGS:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_DEBUG_FLAGS:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
debug_flags=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_DEBUG_FLAGS:/,/case APPLE80211_IOC_LAST_RX_PKT_DATA:/p')
printf '%s\n' "$debug_flags" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$debug_flags" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$debug_flags" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$debug_flags" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$debug_flags" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)DEBUG_FLAGS[[:space:]]*\(' "$source"; then
    fail 'a local DEBUG_FLAGS producer was introduced'
fi
if grep -Eq 'APPLE80211_IOC_DEBUG_FLAGS|kIocDebugFlags|case[[:space:]]+52' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_debug_flags_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe DEBUG_FLAGS SET fixed-stub contract: PASS\n'
