#!/usr/bin/env python3
"""Generate and verify public AWDL OUI/top-master fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_oui_top_master_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-578-skywalk-public-awdl-oui-top-master-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-oui-top-master-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_OUI", 131, 7137, "0xffffff80021bf764", 7155, "0xffffff80021c4580"),
    ("AWDL_TOP_MASTER", 133, 7571, "0xffffff80021bf7c4", 7608, "0xffffff80021c45e0"),
)
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-oui-top-master-fixed-stubs-alignment-v1",
        "source_base_revision": "a3167387f374d9be7168b5f86895ba81fdba25a3",
        "reference": {
        ...
        "checks": {
            ...
            "reference_raw_records_all_four_exact_fixed_leaves": (
                all(token in raw for token in raw_surface_tokens)
                and raw.count("get_body_hex=554889e5b80e2882e05dc3") == 2
                and raw.count("set_body_hex=554889e5b80e2882e05dc3") == 2
                and raw.count("get_reads_neither_public_argument=true") == 2
                and raw.count("set_reads_neither_public_argument=true") == 2
                and raw.count("get_n_type=0x0f") == 2
                and raw.count("set_n_type=0x0f") == 2
            ),
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL OUI identity or top-master state",
                    "No AWDL_MASTER_CHANNEL, AWDL_SYNC_STATE, AWDL_RSDB_CAPS, AWDL_BSSID, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            ...
            "null_fallback_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_AWDL_OUI:")
            ),
            ...
            "no_card_specific_awdl_oui_or_top_master_route_is_introduced": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL OUI/top-master alignment checks failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL OUI/top-master validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
