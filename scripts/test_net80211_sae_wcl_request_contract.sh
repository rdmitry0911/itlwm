...
require(begin, "ieee80211_sae_wcl_request_clear_locked(ic)",
...
ordered(replacement, "controlled replacement preserves its one bind handoff",
        ...
        "IEEE80211_SAE_WCL_REQUEST_PENDING",
        "ic->ic_sae_wcl_request.association_epoch = 0",
        "else", "ieee80211_sae_wcl_request_clear_locked(ic)")
destroy = body(proto_c, "void\nieee80211_pae_selected_bss_lock_destroy",
               "terminal lock destruction")
require(destroy, "ieee80211_sae_wcl_request_clear_locked(ic)",
...
class RequestModel:
    ...
    def resume(self):
        if self.join_active or self.phase != self.PENDING:
    ...
    def bind(self, epoch, identity, profile):
        if self.phase != self.SCAN_ISSUED or identity != self.identity or \
    ...
    def bound_current(self, epoch, identity):
        return self.phase == self.BOUND and self.epoch == epoch and \
            self.identity == identity
