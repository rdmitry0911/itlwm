#!/usr/bin/env python3
"""Generate and verify Skywalk public MIMO_POWERSAVE GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_mimo_powersave_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-558-skywalk-public-mimo-powersave-get-fixed-stub-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-mimo-powersave-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-mimo-powersave-get-fixed-stub-alignment-v1",
        "source_base_revision": "23a993de775dcd53792558090e1ecaeef4e314c0",
        "reference": {
            "bootkc_sha256": "eb5691e94b750df8316f8474245966e02d1badd696f78aa27f003766c9bff06d",
            "bootkc_uuid": "F0ACEF59-61D0-DEDC-C1D2-BECE30DD94E5",
            "embedded_kext_uuid": "8FB4B7F0-D656-3539-B8D6-C1327A50377C",
            "nlist_index": 7505,
            "public_wrapper": "0xffffff80021beec2",
            "next_symbol": "0xffffff80021beecd",
        ...
        "scope": {
            ...
            "mimo_powersave_set_modified": False,
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z27apple80211getMIMO_POWERSAVEP23IO80211SkywalkInterfacePv",
                "nlist_index=7505", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021beec2", "symbol_vmaddr_end=0xffffff80021beecd",
                "symbol_fileoff=0x20beec2", "symbol_fileoff_end=0x20beecd", "symbol_bytes=0x0b",
                "next_nlist_index=6999", "__Z16apple80211getMCSP23IO80211SkywalkInterfaceP19apple80211_mcs_data",
                "next_symbol_vmaddr=0xffffff80021beecd",
                "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
                "0xe082280e", "reads neither public argument",
            )),
            "note_records_scope_and_nonclaims": all(token in note for token in (
                "IOC 56", "0xffffff80021beec2", "0xe082280e", "compile-time Tahoe-only case",
                "selector remains absent from the pre-26 switch", "No local MIMO_POWERSAVE carrier contract is inferred",
                "card-specific route has no MIMO_POWERSAVE entry",
                "does not claim outer-null dispatch behavior, a MIMO_POWERSAVE carrier contract, SET behavior, MIMO-powersave behavior, V1, Virtual IOCTL, card-specific behavior, firmware, runtime-execution, radio, association, traffic, or broader Tahoe behavior parity",
                "No private carrier or selector is constructed or invoked",
            )),
            "public_tahoe_get_returns_exact_unread_status": (
                dispatcher.count("case APPLE80211_IOC_MIMO_POWERSAVE:") == 1
                and "if (cmd == SIOCGA80211)" in mimo_powersave
                and "return static_cast<IOReturn>(0xe082280e);" in mimo_powersave
                and "req->req_data" not in mimo_powersave
                and "return kIOReturnSuccess;" not in mimo_powersave
            ),
            "tahoe_nonget_and_pre26_boundaries_remain_explicit": (
                "return kIOReturnUnsupported;" in mimo_powersave
                and "SIOCSA80211" not in mimo_powersave
                and "#endif // __IO80211_TARGET >= __MAC_26_0" in mimo_powersave
                and "case APPLE80211_IOC_MIMO_POWERSAVE:" not in pre26_dispatcher
            ),
            "outer_null_and_bsd_boundaries_remain_explicit": (
                "if (req == NULL)\n        return kIOReturnUnsupported;" in dispatcher
                and dispatcher.index("if (req == NULL)\n        return kIOReturnUnsupported;") < dispatcher.index("case APPLE80211_IOC_MIMO_POWERSAVE:")
            ...
            "legacy_v1_virtual_and_card_boundaries_remain_explicit": (
                "APPLE80211_IOC_MIMO_POWERSAVE" not in v1
                and "APPLE80211_IOC_MIMO_POWERSAVE" not in virtual
                and "kIocMimoPowersave" not in routes
                and "default:\n            return false;" in routes
                and "routeTahoeSkywalkIoctl(interface, &req," in card
            ),
            "selector_and_carrier_boundaries_are_explicit": (
                "#define APPLE80211_IOC_MIMO_POWERSAVE            56" in ioctl
                and "req->req_data" not in mimo_powersave
            ),
            "all_active_skywalk_source_phases_and_target_guard_are_explicit": (
                all(marker in project for marker in markers)
                and "case APPLE80211_IOC_MIMO_POWERSAVE:" in tahoe_block
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public MIMO_POWERSAVE GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public MIMO_POWERSAVE GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
