...
raw="$root/docs/reference/artifacts/skywalk-rx-antenna-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "RX_ANTENNA SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z23apple80211setRX_ANTENNAP23IO80211SkywalkInterfacePv' \
    'nlist_index=7274' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3acc' \
    'symbol_vmaddr_end=0xffffff80021c3ad7' \
    'symbol_fileoff=0x20c3acc' \
    'symbol_fileoff_end=0x20c3ad7' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7733' \
    'next_symbol=__Z30apple80211setANTENNA_DIVERSITYP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3ad7' \
    'next_symbol_fileoff=0x20c3ad7' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_RX_ANTENNA[[:space:]]+38([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 38'
...
')
rx_case_line=$(line_of 'case APPLE80211_IOC_RX_ANTENNA:')
[ -n "$outer_null_line" ] && [ -n "$rx_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$rx_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_RX_ANTENNA:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_RX_ANTENNA:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
rx=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_RX_ANTENNA:/,/case APPLE80211_IOC_ANTENNA_DIVERSITY:/p')
printf '%s\n' "$rx" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$rx" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$rx" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$rx" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$rx" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)RX_ANTENNA[[:space:]]*\(' "$source"; then
    fail 'a local RX_ANTENNA producer was introduced'
fi
if grep -Eq 'RX_ANTENNA|kIocRxAntenna|case[[:space:]]+38' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_rx_antenna_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe RX_ANTENNA SET fixed-stub contract: PASS\n'
