#!/usr/bin/env python3
"""Generate and verify public AWDL count/master-db/battery fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_counts_master_db_battery_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-582-skywalk-public-awdl-counts-master-db-battery-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-counts-master-db-battery-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_ELECTION_MASTER_COUNTS", 137, 8154, "0xffffff80021bf8ce", 8165, "0xffffff80021c46ea"),
    ("AWDL_GET_AWDL_MASTER_DATABASE", 142, 8187, "0xffffff80021bf905", 8193, "0xffffff80021c4721"),
    ("AWDL_BATTERY_LEVEL", 144, 7767, "0xffffff80021bf91b", 7804, "0xffffff80021c479c"),
)
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-counts-master-db-battery-fixed-stubs-alignment-v1",
        "source_base_revision": "03a5ef9d1fdd5b7a30e5634e0bc2436ec94a9850",
        "reference": {
        ...
        "checks": {
            ...
            "reference_raw_records_all_six_exact_fixed_leaves": all(
                token in raw for token in raw_surface_tokens
            )
            and raw.count("get_body_hex=554889e5b80e2882e05dc3") == 3
            and raw.count("set_body_hex=554889e5b80e2882e05dc3") == 3
            and raw.count("get_reads_neither_public_argument=true") == 3
            and raw.count("set_reads_neither_public_argument=true") == 3
            and raw.count("get_n_type=0x0f") == 3
            and raw.count("set_n_type=0x0f") == 3,
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL election-count, master-database, or battery-level state",
                    "No PEER_CACHE_CONTROL, AWDL timing, AWDL Bluetooth coexistence, AWDL_OOB_REQUEST, AWDL_SYNC_FRAME_TEMPLATE, VHT, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            ...
            < dispatcher.index("case APPLE80211_IOC_AWDL_ELECTION_MASTER_COUNTS:"),
            ...
            "no_card_specific_awdl_count_master_db_or_battery_route_is_introduced": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL counts/master-db/battery alignment checks failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL counts/master-db/battery validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
