#!/usr/bin/env python3
"""Generate and verify public AWDL AP-alignment fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_ap_alignment_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-577-skywalk-public-awdl-ap-alignment-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-ap-alignment-fixed-stubs-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-ap-alignment-fixed-stubs-alignment-v1",
        "source_base_revision": "ef9cbb5e542988138f889509d214b2110c365d6a",
        "reference": {
        ...
        "checks": {
            ...
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, retain, or synthesize AWDL availability-window or sync-frame AP alignment state",
                    "No AWDL_SYNCHRONIZATION_CHANNEL_SEQUENCE, AWDL_MASTER_CHANNEL, AWDL_BSSID, AWDL_ELECTION_ID, virtual-interface, card-specific, legacy V1, firmware, deployment, runtime-selector, association, radio, or traffic claim is made",
                )
            ),
            ...
            "null_fallback_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_AWDL_AVAILABILITY_WINDOW_AP_ALIGNMENT:")
            ),
            ...
            "no_card_specific_awdl_ap_alignment_route_is_introduced": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL AP-alignment checks failed: " + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL AP-alignment validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
