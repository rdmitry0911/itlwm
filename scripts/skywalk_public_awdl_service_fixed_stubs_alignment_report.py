#!/usr/bin/env python3
"""Generate and verify Skywalk public AWDL service fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_awdl_service_fixed_stubs_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-574-skywalk-public-awdl-service-fixed-stubs-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-public-awdl-service-fixed-stubs-bootkc-current/raw.txt"
...
SURFACES = (
    ("AWDL_SERVICE_PARAMS", 119, 7844, "0xffffff80021bf59d", 7871, "0xffffff80021c43d4"),
    (
...
def report():
    ...
    return {
        "schema": "itlwm-skywalk-public-awdl-service-fixed-stubs-alignment-v1",
        "source_base_revision": "638f6f0a2ff002a65b469d53e5e23349287f48dd",
        "reference": {
        ...
        "checks": {
            ...
            "reference_raw_records_all_six_exact_fixed_leaves": (
            ...
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not allocate, inspect, or synthesize an AWDL service, peer-request, or election-algorithm carrier",
                    "No AWDL_ELECTION_ID, AWDL_SYNC_PARAMS, AWDL_SYNC_ENABLED, legacy V1, virtual-interface, card-specific, firmware, deployment, runtime-selector, association, radio, or traffic claim",
                )
            ),
            ...
            "null_fallback_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_AWDL_SERVICE_PARAMS:")
            ),
            ...
            "no_card_specific_awdl_service_route_is_introduced": all(
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public AWDL service fixed-stub alignment checks failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public AWDL service alignment validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
