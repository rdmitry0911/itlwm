#!/usr/bin/env python3
"""Generate and verify Skywalk public P2P_ENABLE GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_p2p_enable_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-564-skywalk-public-p2p-enable-get-fixed-stub-alignment-20260716.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-p2p-enable-get-public-fixed-stub-bootkc-current/raw.txt"
...
MACRO = "APPLE80211_IOC_P2P_ENABLE"
VALUE = 91
...
def report():
    ...
    tahoe_block = section(dispatcher, "#if __IO80211_TARGET >= __MAC_26_0\n        case APPLE80211_IOC_P2P_ENABLE:", "        case APPLE80211_IOC_SOFTAP_PARAMS:")
    group = section(dispatcher, "case APPLE80211_IOC_P2P_ENABLE:", "        case APPLE80211_IOC_SOFTAP_PARAMS:")
    ...
    return {
        "schema": "itlwm-skywalk-public-p2p-enable-get-fixed-stub-alignment-v1",
        "source_base_revision": "5c96dd576decbdef55896d9b1accec606791f15e",
        "reference": {
            ...
            "selector": {"name": "P2P_ENABLE", "ioc": VALUE, "nlist_index": 7240, "public_wrapper": "0xffffff80021bf264", "fileoff": "0x20bf264", "symbol": "__Z23apple80211getP2P_ENABLEP23IO80211SkywalkInterfaceP26apple80211_p2p_enable_data", "next_nlist_index": 7241, "next_symbol": "__Z23apple80211getP2P_LISTENP23IO80211SkywalkInterfacePv"},
        },
        "scope": {
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "selector=P2P_ENABLE", "nlist_index=7240", "symbol_vmaddr=0xffffff80021bf264",
                "symbol_fileoff=0x20bf264", "symbol_bytes=0x0b", "next_nlist_index=7241",
            ...
            "note_records_scope_and_nonclaims": all(token in note for token in (
                "IOC 91", "0xffffff80021bf264", "0xe082280e", "compile-time Tahoe-only case",
                "selector remains absent from the pre-26 Skywalk route, V1, Virtual IOCTL, and card-specific route",
                "No local carrier contract is inferred",
                "does not claim outer-null dispatch behavior, a carrier contract, P2P-enable behavior, SET behavior, P2P listen/scan behavior, V1, Virtual IOCTL, card-specific behavior, firmware, runtime-execution, radio, association, traffic, or broader Tahoe behavior parity",
            ...
            "legacy_virtual_and_card_boundaries_remain_explicit": (
                MACRO not in v1 and MACRO not in virtual and MACRO not in routes
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public P2P_ENABLE GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public P2P_ENABLE GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
