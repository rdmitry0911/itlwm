#!/usr/bin/env python3
"""Generate and verify Skywalk public SHORT_RETRY_LIMIT GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_short_retry_limit_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-547-skywalk-public-short-retry-limit-get-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-short-retry-limit-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    retry = section(dispatcher, "case APPLE80211_IOC_SHORT_RETRY_LIMIT:", "        case APPLE80211_IOC_POWER:")
    ...
    return {
        ...
        "reference": {
            ...
            "nlist_index": 7721,
            "public_wrapper": "0xffffff80021bead1",
            "next_symbol": "0xffffff80021beadc",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z30apple80211getSHORT_RETRY_LIMITP23IO80211SkywalkInterfacePv",
                "nlist_index=7721", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021bead1", "symbol_vmaddr_end=0xffffff80021beadc",
                "symbol_fileoff=0x20bead1", "symbol_fileoff_end=0x20beadc", "symbol_bytes=0x0b",
                "next_nlist_index=7657", "__Z29apple80211getLONG_RETRY_LIMITP23IO80211SkywalkInterfacePv",
                "next_symbol_vmaddr=0xffffff80021beadc", "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
            ...
            "all_active_skywalk_source_phases_and_target_guard_are_explicit": all(marker in project for marker in markers) and "case APPLE80211_IOC_SHORT_RETRY_LIMIT:" in tahoe_block and "#endif // __IO80211_TARGET >= __MAC_26_0" in tahoe_block,
        },
    }
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public SHORT_RETRY_LIMIT GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public SHORT_RETRY_LIMIT GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
