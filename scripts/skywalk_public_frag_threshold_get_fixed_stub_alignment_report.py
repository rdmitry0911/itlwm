#!/usr/bin/env python3
"""Generate and verify Skywalk public FRAG_THRESHOLD GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_frag_threshold_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-544-skywalk-public-frag-threshold-get-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-frag-threshold-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    return {
        ...
        "reference": {
            ...
            "nlist_index": 7499,
            "public_wrapper": "0xffffff80021bea5b",
            "next_symbol": "0xffffff80021bea66",
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(token in raw for token in (
                ...
                "symbol=__Z27apple80211getFRAG_THRESHOLDP23IO80211SkywalkInterfacePv",
                "nlist_index=7499", "n_type=0x0f", "n_sect=1", "n_desc=0x0000",
                "symbol_vmaddr=0xffffff80021bea5b", "symbol_vmaddr_end=0xffffff80021bea66",
                "symbol_fileoff=0x20bea5b", "symbol_fileoff_end=0x20bea66", "symbol_bytes=0x0b",
                "next_nlist_index=7147", "__Z21apple80211getRATE_SETP23IO80211SkywalkInterfaceP24apple80211_rate_set_data",
                "next_symbol_vmaddr=0xffffff80021bea66", "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
            ...
            "outer_null_and_bsd_boundaries_remain_explicit": (
                ...
                and dispatcher.index("if (req == NULL)\n        return kIOReturnUnsupported;") < dispatcher.index("case APPLE80211_IOC_FRAG_THRESHOLD:")
            ...
            "legacy_virtual_and_card_boundaries_remain_separate": (
                "APPLE80211_IOC_FRAG_THRESHOLD" not in v1
                and "APPLE80211_IOC_FRAG_THRESHOLD" not in virtual
                and "kIocFragThreshold" not in routes
                and "APPLE80211_IOC_FRAG_THRESHOLD" not in routes
                and "default:\n            return false;" in routes
                and "routeTahoeSkywalkIoctl(interface, &req," in card
            ),
            "selector_is_declared_and_historical_carrier_is_unconsumed": (
                "#define APPLE80211_IOC_FRAG_THRESHOLD            31" in ioctl
                and "struct apple80211_frag_threshold_data" in ioctl
                and "req->req_data" not in frag
                and "apple80211_frag_threshold_data" not in frag
            ),
            "all_active_skywalk_source_phases_and_target_guard_are_explicit": (
                all(marker in project for marker in markers)
                and "case APPLE80211_IOC_FRAG_THRESHOLD:" in tahoe_block
...
def main():
    ...
    if failed:
        raise ValueError("Skywalk public FRAG_THRESHOLD GET alignment validation failed: " + ", ".join(failed))
...
if __name__ == "__main__":
    try:
        ...
        print(f"Skywalk public FRAG_THRESHOLD GET alignment validation failed: {exc}", file=sys.stderr)
        sys.exit(1)
