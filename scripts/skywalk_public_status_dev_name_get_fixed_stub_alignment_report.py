#!/usr/bin/env python3
"""Generate and verify Skywalk public STATUS_DEV_NAME GET fixed-stub evidence."""
...
OUTPUT = ROOT / "evidence/state/skywalk_public_status_dev_name_get_fixed_stub_alignment_report.json"
NOTE = ROOT / "docs/reference/CR-539-skywalk-public-status-dev-name-get-fixed-stub-alignment-20260715.md"
RAW = ROOT / "docs/reference/artifacts/skywalk-status-dev-name-get-public-fixed-stub-bootkc-current/raw.txt"
...
def report():
    ...
    after_tahoe_branch = status_case[
        status_case.index("#endif // __IO80211_TARGET >= __MAC_26_0")
    ...
    return {
        "schema": "itlwm-skywalk-public-status-dev-name-get-fixed-stub-alignment-v1",
        "source_base_revision": "75b681d556ccf9e457a60368824906296690293f",
        "reference": {
            "bootkc_sha256": "eb5691e94b750df8316f8474245966e02d1badd696f78aa27f003766c9bff06d",
            "bootkc_uuid": "F0ACEF59-61D0-DEDC-C1D2-BECE30DD94E5",
            "embedded_kext_uuid": "8FB4B7F0-D656-3539-B8D6-C1327A50377C",
            "nlist_index": 7595,
            "public_wrapper": "0xffffff80021be92c",
            "next_symbol": "0xffffff80021be937",
        ...
        "scope": {
            ...
            "status_dev_name_set_modified": False,
        ...
        "checks": {
            ...
            "reference_raw_has_exact_unread_public_get_fixed_stub": all(
                ...
                for token in (
                    ...
                    "symbol=__Z28apple80211getSTATUS_DEV_NAMEP23IO80211SkywalkInterfaceP26apple80211_status_dev_data",
                    "nlist_index=7595",
                    "n_type=0x0f",
                    "n_sect=1",
                    "n_desc=0x0000",
                    "symbol_vmaddr=0xffffff80021be92c",
                    "symbol_vmaddr_end=0xffffff80021be937",
                    "symbol_fileoff=0x20be92c",
                    "symbol_fileoff_end=0x20be937",
                    "symbol_bytes=0x0b",
                    "next_nlist_index=7191",
                    "__Z22apple80211getIBSS_MODEP23IO80211SkywalkInterfaceP23apple80211_network_data",
                    "next_symbol_vmaddr=0xffffff80021be937",
                    "body_sha256=9e4580f0175946d7624b2451e6ffda84a93e91e3e2d852d7a2c7998ee2d78576",
                    "0xe082280e",
            ...
            "note_records_scope_and_nonclaims": all(
                ...
                for token in (
                    ...
                    "does not consume the existing local STATUS_DEV_NAME carrier declaration",
                    "card-specific route names IOC 23",
                    "does not claim outer-null dispatch behavior, a STATUS_DEV_NAME carrier ABI, SET behavior, V1, Virtual IOCTL, card-specific behavior, device-name discovery, firmware, runtime-execution, radio, association, traffic, or broader Tahoe behavior parity",
                    "No private carrier or selector is constructed or invoked",
                )
            ),
            "public_tahoe_get_returns_exact_unread_status": (
                dispatcher.count("case APPLE80211_IOC_STATUS_DEV_NAME:") == 1
                ...
                and "req->req_len" not in tahoe_branch
                and "return kIOReturnSuccess;" not in tahoe_branch
            ),
            "tahoe_nonget_and_pre26_boundaries_remain_explicit": (
                "return kIOReturnUnsupported;" in after_tahoe_branch
                and "SIOCSA80211" not in tahoe_branch
                and "#if __IO80211_TARGET >= __MAC_26_0" in status_case
                and "#endif // __IO80211_TARGET >= __MAC_26_0" in status_case
            ),
            "outer_null_dispatch_remains_outside_this_layer": (
                ...
                < dispatcher.index("case APPLE80211_IOC_STATUS_DEV_NAME:")
            ...
            "legacy_virtual_and_card_boundaries_remain_separate": (
                "APPLE80211_IOC_STATUS_DEV_NAME" not in v1
                and "APPLE80211_IOC_STATUS_DEV_NAME" not in virtual
                and "kIocStatusDevName" not in routes
                and "APPLE80211_IOC_STATUS_DEV_NAME" not in routes
                and "default:\n            return false;" in routes
                and "routeTahoeSkywalkIoctl(interface, &req," in card_specific
            ),
            "selector_and_existing_local_carrier_are_not_consumed": (
                "#define APPLE80211_IOC_STATUS_DEV_NAME          23" in ioctl
                and "struct apple80211_status_dev_data" in ioctl
                and "apple80211_status_dev_data" not in tahoe_branch
                and "req->req_data" not in tahoe_branch
            ),
...
def main():
    ...
    if failed:
        raise ValueError(
            "Skywalk public STATUS_DEV_NAME GET alignment validation failed: "
            + ", ".join(failed)
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Skywalk public STATUS_DEV_NAME GET alignment validation failed: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
