...
raw="$root/docs/reference/artifacts/skywalk-long-retry-limit-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "LONG_RETRY_LIMIT SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z29apple80211setLONG_RETRY_LIMITP23IO80211SkywalkInterfacePv' \
    'nlist_index=7685' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3ab6' \
    'symbol_vmaddr_end=0xffffff80021c3ac1' \
    'symbol_fileoff=0x20c3ab6' \
    'symbol_fileoff_end=0x20c3ac1' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7279' \
    'next_symbol=__Z23apple80211setTX_ANTENNAP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3ac1' \
    'next_symbol_fileoff=0x20c3ac1' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_LONG_RETRY_LIMIT[[:space:]]+36([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 36'
...
')
retry_case_line=$(line_of 'case APPLE80211_IOC_LONG_RETRY_LIMIT:')
...
case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_LONG_RETRY_LIMIT:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_LONG_RETRY_LIMIT:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
retry=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_LONG_RETRY_LIMIT:/,/case APPLE80211_IOC_TX_ANTENNA:/p')
...
if grep -Eq '(get|set)LONG_RETRY_LIMIT[[:space:]]*\(' "$source"; then
    fail 'a local LONG_RETRY_LIMIT producer was introduced'
fi
if grep -Eq 'LONG_RETRY_LIMIT|kIocLongRetryLimit|case[[:space:]]+36' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_long_retry_limit_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe LONG_RETRY_LIMIT SET fixed-stub contract: PASS\n'
