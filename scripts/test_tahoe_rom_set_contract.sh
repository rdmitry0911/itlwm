...
raw="$root/docs/reference/artifacts/skywalk-rom-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "ROM SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z16apple80211setROMP23IO80211SkywalkInterfacePv' \
    'nlist_index=7007' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3ae2' \
    'symbol_vmaddr_end=0xffffff80021c3aed' \
    'symbol_fileoff=0x20c3ae2' \
    'symbol_fileoff_end=0x20c3aed' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7158' \
    'next_symbol=__Z21apple80211setDTIM_INTP23IO80211SkywalkInterfaceP24apple80211_dtim_int_data' \
    'next_symbol_vmaddr=0xffffff80021c3aed' \
    'next_symbol_fileoff=0x20c3aed' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_ROM[[:space:]]+40([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 40'
...
')
rom_case_line=$(line_of 'case APPLE80211_IOC_ROM:')
[ -n "$outer_null_line" ] && [ -n "$rom_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$rom_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_ROM:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_ROM:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
rom=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_ROM:/,/case APPLE80211_IOC_RAND:/p')
printf '%s\n' "$rom" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$rom" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$rom" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$rom" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$rom" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)ROM[[:space:]]*\(' "$source"; then
    fail 'a local ROM producer was introduced'
fi
if grep -Eq 'APPLE80211_IOC_ROM|kIocRom|case[[:space:]]+40' "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_rom_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe ROM SET fixed-stub contract: PASS\n'
