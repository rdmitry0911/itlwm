...
PRIOR_BLOCKER_EVIDENCE = Path("evidence/runtime/tahoe_auth_mgmt_tx_enqueue_drain_boundary.json")
AUTH_SUBTYPE = "0xb0"
IWN_FILTER_BSS = 0x20
...
def load_prior_blocker_signature():
    if not PRIOR_BLOCKER_EVIDENCE.exists():
        return {
            "source": str(PRIOR_BLOCKER_EVIDENCE),
            "observed": False,
            "events": [],
        }
    try:
        data = json.loads(PRIOR_BLOCKER_EVIDENCE.read_text(encoding="utf-8"))
    except Exception:
        return {
            "source": str(PRIOR_BLOCKER_EVIDENCE),
            "observed": False,
    ...
    events = []
    for event in (((data.get("auth") or {}).get("iwn_tx_path_events")) or []):
        fields = event.get("fields") or {}
        txid = int_field(fields, "txid")
        if fields.get("auth_seq") == "0x0001" and txid == 15:
            events.append(
                {
                    "source": event.get("source", "committed_runtime_evidence"),
                    "line": event.get("line", ""),
                    "fields": fields,
                }
            )
...
def parse_auth_markers(log_output, ioreg_output, nonce, allowed_ssid, allowed_psk):
    ...
    for line in redacted_log_lines:
        ...
        if "iwn_auth: add_bss_node OK" in line:
            bss_node_events.append(
                {
                    "source": "guest_unified_log",
                    "line": line,
                    "fields": parse_kv_blob(line),
    ...
    bss_node_property = ioreg_properties.get("itlwm-iwn-auth-bss-node")
    if bss_node_property:
        bss_node_events.append(
            {
                "source": "guest_ioreg",
                "line": bss_node_property["value"],
                "fields": bss_node_property["fields"],
            }
        )
    ...
    for event in tx_path_events:
        ...
        tx_peer_unicast = (
            unicast_field == "1" if unicast_field is not None else txid_not_broadcast
        )
        auth_tx_identity_events.append(
            {
                "event": event,
                "txid": txid,
    ...
    selected_tx_identity = None
    for identity in reversed(auth_tx_identity_events):
        if identity["txid_not_broadcast"] and identity["tx_peer_unicast"]:
            selected_tx_identity = identity
            break
    if selected_tx_identity is None and auth_tx_identity_events:
        selected_tx_identity = auth_tx_identity_events[-1]
    ...
    return {
        ...
        "bss_node_events": bss_node_events[-8:],
        "tx_path_events": tx_path_events[-8:],
        "tx_identity_events": [event["event"] for event in auth_tx_identity_events[-8:]],
        "txid": selected_tx_identity["txid"] if selected_tx_identity else None,
        "broadcast_txid": selected_tx_identity["broadcast_txid"]
        if selected_tx_identity
        else None,
        "txid_not_broadcast": bool(
            selected_tx_identity and selected_tx_identity["txid_not_broadcast"]
        ),
        "tx_peer_unicast": bool(
            selected_tx_identity and selected_tx_identity["tx_peer_unicast"]
        ),
...
def build_evidence(args):
    ...
    nonce = "itlwm-auth-unicast-tx-" + uuid.uuid4().hex[:16]
    prior_blocker = load_prior_blocker_signature()
    ...
    join_result = base.run_guest_script(
        args,
        "guest CoreWLAN directed join trigger for AUTH management TX enqueue/drain",
    ...
    ready = (
        ...
        and isinstance(auth_markers["txid"], int)
        and auth_markers["txid"] <= 14
    )
    ...
    evidence = {
        ...
        "auth": {
            ...
            "response_window_observable": auth_markers["response_window_observable"],
            ...
            "prior_broadcast_txid_signature": prior_blocker,
            "txid": auth_markers["txid"],
            "broadcast_txid": auth_markers["broadcast_txid"],
            ...
            "bss_node_events": auth_markers["bss_node_events"],
        ...
        "candidate_fix": {
            ...
            "source_delta_paths": [
                "itlwm/hal_iwn/ItlIwn.hpp",
                "itlwm/hal_iwn/ItlIwn.cpp",
                "scripts/capture_tahoe_auth_unicast_tx_node.py",
        ...
        "verdict": {
            ...
            "reason": "live Tahoe guest proved AUTH(seq=1) entered iwn TX as a unicast peer firmware node after the prior txid=15 blocker"
            if ready
...
def main():
    ...
    print(
        ...
        + " unicast="
        + str(evidence["auth"]["tx_peer_unicast"]).lower()
        + " identifier="
        + evidence["runtime_environment"]["allowed_ap_identifier"]
    )
    return 0
