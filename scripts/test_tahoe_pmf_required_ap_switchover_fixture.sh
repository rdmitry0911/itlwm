...
# The final required PID/AP attestation is later than rekey's post-command
# network sample.  Fake iw retains its generated pinned output but changes only
# private fake-network state at that third rekey AP-shape read.  A rekey
# success witness must not be published from the stale baseline.
STATE_DIR="$(mktemp -d "$STATE_PREFIX"XXXXXX)"
"$HELPER" --activate --state-dir "$STATE_DIR" --lease-seconds 60 \
    >"$TMP_ROOT/rekey-postfinalrequired-network-activate.out" \
    2>"$TMP_ROOT/rekey-postfinalrequired-network-activate.err" ||
    fail 'post-final-required network rekey fixture could not activate required PMF'
REKEY_CLI_LINES_BEFORE="$(wc -l <"$FAKE_CLI_LOG")"
IW_CALLS_BEFORE="$(tr -d '[:space:]' <"$FAKE_IW_CALL_COUNT")"
case "$IW_CALLS_BEFORE" in ''|*[!0-9]*) fail 'fake iw call counter is invalid';; esac
FINAL_REKEY_POSTFINALREQUIRED_NETWORK_IW_CALL=$((IW_CALLS_BEFORE + 3))
if FAKE_MUTATE_NETWORK_ON_IW_CALL="$FINAL_REKEY_POSTFINALREQUIRED_NETWORK_IW_CALL" \
    "$HELPER" --rekey --state-dir "$STATE_DIR" \
    >"$TMP_ROOT/rekey-postfinalrequired-network.out" \
    2>"$TMP_ROOT/rekey-postfinalrequired-network.err"; then
    fail 'group rekey accepted network drift after final required attestation'
fi
grep -Fq 'host network invariants changed before rekey success publication' \
    "$TMP_ROOT/rekey-postfinalrequired-network.err" ||
    fail 'post-final-required network drift did not retain its categorical diagnostic'
[ "$(wc -l <"$FAKE_CLI_LOG")" -eq $((REKEY_CLI_LINES_BEFORE + 1)) ] ||
    fail 'post-final-required network drift did not reach exactly one hostapd CLI request'
[ ! -e "$STATE_DIR/rekey.status" ] ||
    fail 'post-final-required network drift wrote a success witness'
grep -Fxq 'rekey_attempted=true' "$STATE_DIR/rekey.requested" ||
    fail 'post-final-required network drift did not retain its one-shot receipt'
[ -r "$RUN_DIR/hostapd-5g-pmf-required.pid" ] ||
    fail 'post-final-required network drift disturbed required hostapd before rollback'
[ -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-final-required network drift cleared the required-state marker'
[ -r "$STATE_DIR/watchdog.pid" ] ||
    fail 'post-final-required network drift did not retain its watchdog receipt'
FINAL_REKEY_POSTFINALREQUIRED_NETWORK_WATCHDOG_PID="$(tr -d '[:space:]' <"$STATE_DIR/watchdog.pid")"
/bin/kill -0 "$FINAL_REKEY_POSTFINALREQUIRED_NETWORK_WATCHDOG_PID" >/dev/null 2>&1 ||
    fail 'post-final-required network drift did not retain a live watchdog'
printf 'stable\n' >"$FAKE_NETWORK_STATE"
"$HELPER" --rollback --state-dir "$STATE_DIR" \
    >"$TMP_ROOT/rekey-postfinalrequired-network-rollback.out" \
    2>"$TMP_ROOT/rekey-postfinalrequired-network-rollback.err" ||
    fail 'post-final-required network drift did not permit explicit optional rollback'
grep -Fxq 'PMF_AP_ROLLBACK=OPTIONAL_RESTORED' \
    "$TMP_ROOT/rekey-postfinalrequired-network-rollback.out" ||
    fail 'post-final-required network drift rollback did not report optional restoration'
[ -r "$RUN_DIR/hostapd-5g.pid" ] ||
    fail 'post-final-required network drift rollback did not restore optional hostapd'
[ ! -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-final-required network drift rollback left the active marker'
if /bin/kill -0 "$FINAL_REKEY_POSTFINALREQUIRED_NETWORK_WATCHDOG_PID" >/dev/null 2>&1; then
    fail 'post-final-required network drift rollback left the watchdog live'
fi
...
# A post-transition recovery cannot report verified optional rollback if the
# state-bound staged pair changes during its final host-network read.  Fake ip
# appends only to generated required config at that third activation route read.
POSTTRANSITION_CONFIG_STATE_DIR="$(mktemp -d "$STATE_PREFIX"XXXXXX)"
ROUTE_CALLS_BEFORE="$(tr -d '[:space:]' <"$FAKE_ROUTE_CALL_COUNT")"
case "$ROUTE_CALLS_BEFORE" in ''|*[!0-9]*) fail 'fake route call counter is invalid';; esac
POSTTRANSITION_CONFIG_ROUTE_CALL=$((ROUTE_CALLS_BEFORE + 3))
if FAKE_MUTATE_REQUIRED_CONFIG_ON_ROUTE_CALL="$POSTTRANSITION_CONFIG_ROUTE_CALL" \
    FAKE_FAIL_REQUIRED=1 "$HELPER" --activate \
    --state-dir "$POSTTRANSITION_CONFIG_STATE_DIR" --lease-seconds 60 \
    >"$TMP_ROOT/posttransition-config-activate.out" \
    2>"$TMP_ROOT/posttransition-config-activate.err"; then
    fail 'activation accepted a post-transition configuration drift'
fi
grep -Fq 'required-PMF hostapd activation failed; rollback watchdog remains armed' \
    "$TMP_ROOT/posttransition-config-activate.err" ||
    fail 'post-transition recovery accepted configuration drift after final network verification'
! grep -Fq 'optional rollback verified' "$TMP_ROOT/posttransition-config-activate.err" ||
    fail 'post-transition configuration drift claimed verified optional rollback'
[ ! -e "$POSTTRANSITION_CONFIG_STATE_DIR/rollback.status" ] ||
    fail 'post-transition configuration drift wrote a rollback receipt'
[ -r "$RUN_DIR/hostapd-5g.pid" ] ||
    fail 'post-transition configuration drift did not restore optional hostapd'
POSTTRANSITION_CONFIG_OPTIONAL_PID="$(tr -d '[:space:]' <"$RUN_DIR/hostapd-5g.pid")"
/bin/kill -0 "$POSTTRANSITION_CONFIG_OPTIONAL_PID" >/dev/null 2>&1 ||
    fail 'post-transition configuration drift restored no live optional hostapd'
[ ! -e "$RUN_DIR/hostapd-5g-pmf-required.pid" ] ||
    fail 'post-transition configuration drift left required hostapd active'
[ -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition configuration drift cleared the rollback marker'
[ -r "$POSTTRANSITION_CONFIG_STATE_DIR/watchdog.pid" ] ||
    fail 'post-transition configuration drift did not retain its watchdog receipt'
POSTTRANSITION_CONFIG_WATCHDOG_PID="$(tr -d '[:space:]' <"$POSTTRANSITION_CONFIG_STATE_DIR/watchdog.pid")"
/bin/kill -0 "$POSTTRANSITION_CONFIG_WATCHDOG_PID" >/dev/null 2>&1 ||
    fail 'post-transition configuration drift did not retain a live watchdog process'
write_config "$REQUIRED" 2 'WPA-PSK-SHA256' fixture-network
"$HELPER" --rollback --state-dir "$POSTTRANSITION_CONFIG_STATE_DIR" \
    >"$TMP_ROOT/posttransition-config-rollback.out" \
    2>"$TMP_ROOT/posttransition-config-rollback.err" ||
    fail 'stable explicit rollback did not recover post-transition configuration drift'
grep -Fxq 'PMF_AP_ROLLBACK=OPTIONAL_RESTORED' \
    "$TMP_ROOT/posttransition-config-rollback.out" ||
    fail 'post-transition configuration drift rollback did not report optional restoration'
[ ! -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition configuration drift rollback left the active marker'
if /bin/kill -0 "$POSTTRANSITION_CONFIG_WATCHDOG_PID" >/dev/null 2>&1; then
...
# The staged pair must remain current through the post-transition final network
# fence.  Fake ip appends only to generated required config at that fourth
# activation route read, after the prior recovery pair check.
POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR="$(mktemp -d "$STATE_PREFIX"XXXXXX)"
ROUTE_CALLS_BEFORE="$(tr -d '[:space:]' <"$FAKE_ROUTE_CALL_COUNT")"
case "$ROUTE_CALLS_BEFORE" in ''|*[!0-9]*) fail 'fake route call counter is invalid';; esac
POSTTRANSITION_POSTNETWORK_CONFIG_ROUTE_CALL=$((ROUTE_CALLS_BEFORE + 4))
if FAKE_MUTATE_REQUIRED_CONFIG_ON_ROUTE_CALL="$POSTTRANSITION_POSTNETWORK_CONFIG_ROUTE_CALL" \
    FAKE_FAIL_REQUIRED=1 "$HELPER" --activate \
    --state-dir "$POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR" --lease-seconds 60 \
    >"$TMP_ROOT/posttransition-postnetwork-config-activate.out" \
    2>"$TMP_ROOT/posttransition-postnetwork-config-activate.err"; then
    fail 'activation accepted a post-transition post-network configuration drift'
fi
grep -Fq 'required-PMF hostapd activation failed; rollback watchdog remains armed' \
    "$TMP_ROOT/posttransition-postnetwork-config-activate.err" ||
    fail 'post-transition recovery accepted a post-network configuration drift'
! grep -Fq 'optional rollback verified' "$TMP_ROOT/posttransition-postnetwork-config-activate.err" ||
    fail 'post-transition post-network configuration drift claimed verified optional rollback'
[ ! -e "$POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR/rollback.status" ] ||
    fail 'post-transition post-network configuration drift wrote a rollback receipt'
[ -r "$RUN_DIR/hostapd-5g.pid" ] ||
    fail 'post-transition post-network configuration drift did not restore optional hostapd'
POSTTRANSITION_POSTNETWORK_CONFIG_OPTIONAL_PID="$(tr -d '[:space:]' <"$RUN_DIR/hostapd-5g.pid")"
/bin/kill -0 "$POSTTRANSITION_POSTNETWORK_CONFIG_OPTIONAL_PID" >/dev/null 2>&1 ||
    fail 'post-transition post-network configuration drift restored no live optional hostapd'
[ ! -e "$RUN_DIR/hostapd-5g-pmf-required.pid" ] ||
    fail 'post-transition post-network configuration drift left required hostapd active'
[ -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition post-network configuration drift cleared the rollback marker'
[ -r "$POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR/watchdog.pid" ] ||
    fail 'post-transition post-network configuration drift did not retain its watchdog receipt'
POSTTRANSITION_POSTNETWORK_CONFIG_WATCHDOG_PID="$(tr -d '[:space:]' <"$POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR/watchdog.pid")"
/bin/kill -0 "$POSTTRANSITION_POSTNETWORK_CONFIG_WATCHDOG_PID" >/dev/null 2>&1 ||
    fail 'post-transition post-network configuration drift did not retain a live watchdog process'
write_config "$REQUIRED" 2 'WPA-PSK-SHA256' fixture-network
"$HELPER" --rollback --state-dir "$POSTTRANSITION_POSTNETWORK_CONFIG_STATE_DIR" \
    >"$TMP_ROOT/posttransition-postnetwork-config-rollback.out" \
    2>"$TMP_ROOT/posttransition-postnetwork-config-rollback.err" ||
    fail 'stable explicit rollback did not recover post-transition post-network configuration drift'
grep -Fxq 'PMF_AP_ROLLBACK=OPTIONAL_RESTORED' \
    "$TMP_ROOT/posttransition-postnetwork-config-rollback.out" ||
    fail 'post-transition post-network configuration drift rollback did not report optional restoration'
[ ! -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition post-network configuration drift rollback left the active marker'
if /bin/kill -0 "$POSTTRANSITION_POSTNETWORK_CONFIG_WATCHDOG_PID" >/dev/null 2>&1; then
...
# A final optional PID/AP attestation is itself later than the final network
# read.  Fake iw preserves its generated pinned shape but changes only private
# fake-network state at that fifth activation shape probe after required-start
# failure.  Recovery must not release ownership from the stale network sample.
POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR="$(mktemp -d "$STATE_PREFIX"XXXXXX)"
IW_CALLS_BEFORE="$(tr -d '[:space:]' <"$FAKE_IW_CALL_COUNT")"
case "$IW_CALLS_BEFORE" in ''|*[!0-9]*) fail 'fake iw call counter is invalid';; esac
POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_IW_CALL=$((IW_CALLS_BEFORE + 5))
if FAKE_MUTATE_NETWORK_ON_IW_CALL="$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_IW_CALL" \
    FAKE_FAIL_REQUIRED=1 "$HELPER" --activate \
    --state-dir "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR" --lease-seconds 60 \
    >"$TMP_ROOT/posttransition-postfinaloptional-network-activate.out" \
    2>"$TMP_ROOT/posttransition-postfinaloptional-network-activate.err"; then
    fail 'activation accepted a post-transition post-final-optional network drift'
fi
grep -Fq 'required-PMF hostapd activation failed; rollback watchdog remains armed' \
    "$TMP_ROOT/posttransition-postfinaloptional-network-activate.err" ||
    fail 'post-transition recovery accepted network drift after final optional attestation'
! grep -Fq 'optional rollback verified' \
    "$TMP_ROOT/posttransition-postfinaloptional-network-activate.err" ||
    fail 'post-transition post-final-optional network drift claimed verified optional rollback'
[ ! -e "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR/rollback.status" ] ||
    fail 'post-transition post-final-optional network drift wrote a rollback receipt'
[ -r "$RUN_DIR/hostapd-5g.pid" ] ||
    fail 'post-transition post-final-optional network drift did not restore optional hostapd'
POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_OPTIONAL_PID="$(tr -d '[:space:]' <"$RUN_DIR/hostapd-5g.pid")"
/bin/kill -0 "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_OPTIONAL_PID" >/dev/null 2>&1 ||
    fail 'post-transition post-final-optional network drift restored no live optional hostapd'
[ ! -e "$RUN_DIR/hostapd-5g-pmf-required.pid" ] ||
    fail 'post-transition post-final-optional network drift left required hostapd active'
[ -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition post-final-optional network drift cleared the rollback marker'
[ -r "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR/watchdog.pid" ] ||
    fail 'post-transition post-final-optional network drift did not retain its watchdog receipt'
POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_WATCHDOG_PID="$(tr -d '[:space:]' <"$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR/watchdog.pid")"
/bin/kill -0 "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_WATCHDOG_PID" >/dev/null 2>&1 ||
    fail 'post-transition post-final-optional network drift did not retain a live watchdog process'
printf 'stable\n' >"$FAKE_NETWORK_STATE"
"$HELPER" --rollback --state-dir "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_STATE_DIR" \
    >"$TMP_ROOT/posttransition-postfinaloptional-network-rollback.out" \
    2>"$TMP_ROOT/posttransition-postfinaloptional-network-rollback.err" ||
    fail 'stable explicit rollback did not recover post-transition post-final-optional network drift'
grep -Fxq 'PMF_AP_ROLLBACK=OPTIONAL_RESTORED' \
    "$TMP_ROOT/posttransition-postfinaloptional-network-rollback.out" ||
    fail 'post-transition post-final-optional network drift rollback did not report optional restoration'
[ ! -e "$CONTROL_DIR/active.state" ] ||
    fail 'post-transition post-final-optional network drift rollback left the active marker'
if /bin/kill -0 "$POSTTRANSITION_POSTFINALOPTIONAL_NETWORK_WATCHDOG_PID" >/dev/null 2>&1; then
    fail 'post-transition post-final-optional network drift rollback left the watchdog live'
fi
