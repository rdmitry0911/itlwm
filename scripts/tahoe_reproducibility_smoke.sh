...
require_literal "timeout 300s sudo kmutil create --new aux" "$REPRO_DOC" "documented AuxKC rebuild"
require_literal "--bundle-path /Library/Extensions/AirportItlwm.kext" "$REPRO_DOC" "documented AuxKC bundle path"
require_literal "AuxiliaryKernelExtensions.kc.preinstall-bak" "$REPRO_DOC" "documented AuxKC rollback backup"
require_literal "timeout 120s sudo /sbin/reboot" "$REPRO_DOC" "documented reboot envelope"
