...
raw="$root/docs/reference/artifacts/skywalk-frag-threshold-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "FRAG_THRESHOLD SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z27apple80211setFRAG_THRESHOLDP23IO80211SkywalkInterfacePv' \
    'nlist_index=7536' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3a7f' \
    'symbol_vmaddr_end=0xffffff80021c3a8a' \
    'symbol_fileoff=0x20c3a7f' \
    'symbol_fileoff_end=0x20c3a8a' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7165' \
    'next_symbol=__Z21apple80211setRATE_SETP23IO80211SkywalkInterfaceP24apple80211_rate_set_data' \
    'next_symbol_vmaddr=0xffffff80021c3a8a' \
    'next_symbol_fileoff=0x20c3a8a' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_FRAG_THRESHOLD[[:space:]]+31([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 31'
grep -Fq 'struct apple80211_frag_threshold_data' "$header" ||
...
')
frag_case_line=$(line_of 'case APPLE80211_IOC_FRAG_THRESHOLD:')
[ -n "$outer_null_line" ] && [ -n "$frag_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$frag_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_FRAG_THRESHOLD:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_FRAG_THRESHOLD:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
frag=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_FRAG_THRESHOLD:/,/case APPLE80211_IOC_SHORT_SLOT:/p')
printf '%s\n' "$frag" | grep -Fq 'case APPLE80211_IOC_FRAG_THRESHOLD:' ||
    fail 'Tahoe selector case is missing'
printf '%s\n' "$frag" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$frag" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$frag" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$frag" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$frag" | grep -Fq 'apple80211_frag_threshold_data'; then
    fail 'historical header carrier was activated'
fi
if printf '%s\n' "$frag" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)FRAG_THRESHOLD[[:space:]]*\(' "$source"; then
    fail 'a local FRAG_THRESHOLD producer was introduced'
fi
if grep -Eq 'FRAG_THRESHOLD|kIocFragThreshold|case[[:space:]]+31' \
...
python3 "$root/scripts/skywalk_public_frag_threshold_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe FRAG_THRESHOLD SET fixed-stub contract: PASS\n'
