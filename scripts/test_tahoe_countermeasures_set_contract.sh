...
raw="$root/docs/reference/artifacts/skywalk-countermeasures-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "COUNTERMEASURES SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z28apple80211setCOUNTERMEASURESP23IO80211SkywalkInterfacePv' \
    'nlist_index=7611' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3a74' \
    'symbol_vmaddr_end=0xffffff80021c3a7f' \
    'symbol_fileoff=0x20c3a74' \
    'symbol_fileoff_end=0x20c3a7f' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7536' \
    'next_symbol=__Z27apple80211setFRAG_THRESHOLDP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3a7f' \
    'next_symbol_fileoff=0x20c3a7f' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_COUNTERMEASURES[[:space:]]+30([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 30'
...
')
counter_case_line=$(line_of 'case APPLE80211_IOC_COUNTERMEASURES:')
[ -n "$outer_null_line" ] && [ -n "$counter_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$counter_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_COUNTERMEASURES:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_COUNTERMEASURES:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
counter=$(printf '%s\n' "$dispatcher" |
    sed -n '/#if __IO80211_TARGET >= __MAC_26_0$/,/case APPLE80211_IOC_FRAG_THRESHOLD:/p' |
    sed -n '/case APPLE80211_IOC_COUNTERMEASURES:/,/case APPLE80211_IOC_FRAG_THRESHOLD:/p')
printf '%s\n' "$counter" | grep -Fq 'case APPLE80211_IOC_COUNTERMEASURES:' ||
    fail 'Tahoe selector case is missing'
printf '%s\n' "$counter" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$counter" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$counter" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$counter" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$counter" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)COUNTERMEASURES[[:space:]]*\(' "$source"; then
    fail 'a local COUNTERMEASURES producer was introduced'
fi
if grep -Eq 'COUNTERMEASURES|kIocCountermeasures|case[[:space:]]+30' \
    "$v1" "$virtual" "$v2" "$routes"; then
...
python3 "$root/scripts/skywalk_public_countermeasures_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe COUNTERMEASURES SET fixed-stub contract: PASS\n'
