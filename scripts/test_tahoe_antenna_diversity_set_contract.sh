...
raw="$root/docs/reference/artifacts/skywalk-antenna-diversity-set-public-fixed-stub-bootkc-25c56/raw.txt"
manifest="$(dirname -- "$raw")/SHA256SUMS.txt"

fail() {
    echo "ANTENNA_DIVERSITY SET fixed-stub contract: $1" >&2
    exit 1
}
...
for marker in \
    ...
    'symbol=__Z30apple80211setANTENNA_DIVERSITYP23IO80211SkywalkInterfacePv' \
    'nlist_index=7733' \
    'n_type=0x0f' \
    'n_sect=1' \
    'n_desc=0x0000' \
    'symbol_vmaddr=0xffffff80021c3ad7' \
    'symbol_vmaddr_end=0xffffff80021c3ae2' \
    'symbol_fileoff=0x20c3ad7' \
    'symbol_fileoff_end=0x20c3ae2' \
    'symbol_bytes=0x0b' \
    'next_nlist_index=7007' \
    'next_symbol=__Z16apple80211setROMP23IO80211SkywalkInterfacePv' \
    'next_symbol_vmaddr=0xffffff80021c3ae2' \
    'next_symbol_fileoff=0x20c3ae2' \
...
grep -Eq '^#define[[:space:]]+APPLE80211_IOC_ANTENNA_DIVERSITY[[:space:]]+39([[:space:]]|$)' "$header" ||
    fail 'selector no longer resolves to 39'
...
')
diversity_case_line=$(line_of 'case APPLE80211_IOC_ANTENNA_DIVERSITY:')
[ -n "$outer_null_line" ] && [ -n "$diversity_case_line" ] ||
    fail 'canonical dispatcher boundary is missing'
[ "$outer_null_line" -lt "$diversity_case_line" ] ||
    fail 'SET route escaped the existing carrier-null guard'

case_count=$(printf '%s\n' "$dispatcher" | grep -Fc 'case APPLE80211_IOC_ANTENNA_DIVERSITY:' || true)
[ "$case_count" -eq 1 ] || fail 'selector case is not unique'
awk '
/#if __IO80211_TARGET >= __MAC_26_0/ { guarded = 1; next }
/#endif/ { guarded = 0; next }
guarded && /case APPLE80211_IOC_ANTENNA_DIVERSITY:/ { found = 1; exit }
END { exit !found }
' "$source" || fail 'selector case escaped the Tahoe target guard'
diversity=$(printf '%s\n' "$dispatcher" |
    sed -n '/case APPLE80211_IOC_ANTENNA_DIVERSITY:/,/case APPLE80211_IOC_ROM:/p')
printf '%s\n' "$diversity" | grep -Fq 'if (cmd == SIOCGA80211 || cmd == SIOCSA80211)' ||
    fail 'GET and canonical SET directions are no longer paired'
printf '%s\n' "$diversity" | grep -Fq 'return static_cast<IOReturn>(0xe082280e);' ||
    fail 'fixed raw status changed'
printf '%s\n' "$diversity" | grep -Fq 'return kIOReturnUnsupported;' ||
    fail 'unknown command fallback disappeared'
if printf '%s\n' "$diversity" | grep -Eq 'req->(req_data|req_len|req_val)'; then
    fail 'fixed stub now inspects the carrier'
fi
if printf '%s\n' "$diversity" | grep -Fq 'return kIOReturnSuccess;'; then
    fail 'fixed stub became a success acknowledgement'
fi

if grep -Eq '(get|set)ANTENNA_DIVERSITY[[:space:]]*\(' "$source"; then
    fail 'a local ANTENNA_DIVERSITY producer was introduced'
fi
grep -Fq 'IOCTL_GET(request_type, ANTENNA_DIVERSITY, apple80211_antenna_data);' "$v1" ||
    fail 'separate V1 GET route disappeared'
if grep -Fq 'IOCTL_SET(request_type, ANTENNA_DIVERSITY' "$v1"; then
    fail 'a V1 ANTENNA_DIVERSITY setter was introduced'
fi
if grep -Eq 'ANTENNA_DIVERSITY|kIocAntennaDiversity|case[[:space:]]+39' "$virtual" "$v2" "$routes"; then
    fail 'an alternate Virtual, V2, or controller route was introduced'
fi
...
python3 "$root/scripts/skywalk_public_antenna_diversity_get_fixed_stub_alignment_report.py" --check >/dev/null
printf 'Tahoe ANTENNA_DIVERSITY SET fixed-stub contract: PASS\n'
