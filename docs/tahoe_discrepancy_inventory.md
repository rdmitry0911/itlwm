...
owner. The modern `setWCL_ROAM_PROFILE_CONFIG` quarantine is now functionally
closed: the recovered 0x23c per-band brackets are decoded into a generation-
published Intel policy, and shared IWN/IWM/IWX STA paths consume its RSSI
ranges, scan periods/backoff, roam delta, and target-band boost while selecting
autonomous scan candidates. Runtime DTrace proves Tahoe's live carrier selects
the 20-second bracket and WPA3 traffic remains intact. This is a host-side
policy equivalent, not a claim of Broadcom `roam_prof`, `join_pref`, multi-AP
IOVAR, callback, or transport identity. Legacy
`setWCL_LEGACY_ROAM_PROFILE_CONFIG` remains a no-local-backend quarantine: its
distinct RoamAdapter path selects legacy V4/V2 policy, `roam_prof`, and
multi-AP work while the local port had only an unread 0x60 cache. Its direct
null error remains and non-null legacy input returns unsupported before carrier
access. Reassociation remains a separate scope.
...
## item 229 - Tahoe STA_ASSOC_DONE lacked the candidate-matched WCL completion
...
- status: implemented; static contract validated; runtime validation pending
...
- it then reaches `AppleBCMWLANJoinAdapter::handleAssoc`, whose selected-
  candidate ledger publishes the distinct `0xd3` / `0x1c` completion;
- `WCLJoinManager::authAssocCompleteEventHandler` accepts only that present,
  exact-length `0x1c` completion body.

Local closure:

- Tahoe `IEEE80211_EVT_STA_ASSOC_DONE` no longer returns without publication;
- the local path first publishes the generic 8-byte `0x4e` success status,
  then attempts the separate Apple-shaped `0xd3` / `0x1c` JoinAdapter
  completion;
- the `0xd3` publisher requires `S_ASSOC`, a current matching epoch and
  selected-BSS snapshot, an armed unconsumed WCL owner, and exact selected/
  candidate BSSID plus SSID agreement; it claims the lease one time only;
- deauthentication and replacement WCL lifecycle edges clear that lease;
- the legacy non-Tahoe `APPLE80211_M_ASSOC_DONE` path remains unchanged.

Validation boundary:

- static payload layout and source-order gates are deterministic;
- runtime acceptance requires an active WCL candidate to demonstrate the
  ordered `0x4e` then `0xd3` publication and the subsequent JoinManager
  progress;
- no previous generic association or traffic result is evidence that the
  `0xd3` JoinAdapter completion worked.
