...
## 2026-07-15 correction: legacy AWDL sync-params blind-success shim

The historical virtual-interface IOC 116 SET shim previously returned success
while ignoring its typed sync-params carrier. Direct current 25C56
nested-KEXT recovery shows a selector gate, nonzero-status propagation,
AWDL-protocol owner test, and only then a tail dispatch; a failed owner test
returns raw `0xe082280e`. A separate internal handler validates the `0x18`
and non-null request carrier, then performs an AWDLPeerManager metaclass
cast/call, while the local packed carrier remains `0x14`. That is a recorded
size divergence, not a basis for a guessed header change or carrier-layout
claim.

The active legacy shim now returns `kIOReturnUnsupported` without reading
either argument. Its paired GET version result, typed route, and Tahoe's
absence of IOC 116 remain intact. Tahoe does not compile
`AirportVirtualIOCTL.cpp`. This is a historical source-surface fail-closed
boundary, not a Tahoe runtime, Apple terminal-status, carrier-layout,
...
## 2026-07-15 correction: legacy AWDL peer-traffic-registration blind-success shim

The historical virtual-interface IOC 164 SET half previously parsed a local
peer-traffic name and active bit, performed no registration, and then returned
success. Direct current 25C56 nested-KEXT recovery shows a public selector
gate, nonzero-status propagation, AWDL-protocol owner test, and dynamic tail
dispatch; a failed owner test returns raw `0xe082280e`. A separate internal
handler requires a non-null `0xb8` request carrier and directly reaches an
active AWDL-peer-manager terminal, while the packed local x86_64 carrier is
`0xac`. That is a recorded size divergence, not a basis for a guessed header
change or carrier-layout claim.

The active legacy SET half now returns `kIOReturnUnsupported` without reading
either argument. Its bidirectional route, separate legacy GET literal result,
and Tahoe's absence of IOC 164 remain intact. Tahoe does not compile
`AirportVirtualIOCTL.cpp`. This is a historical source-surface fail-closed
boundary, not a Tahoe runtime, Apple terminal-status, carrier-layout,
...
## 2026-07-15 correction: legacy AWDL sync-state blind-success cache

The historical virtual-interface IOC 134 SET half previously copied a state
...
`0x18` request carrier and directly reaches an active AWDL-peer-manager
terminal, while the packed local carrier is `0x08`. That is a recorded size
divergence, not a basis for a guessed header change or carrier-layout claim.

The active legacy SET half now returns `kIOReturnUnsupported` without reading
either argument. Its bidirectional route, cached legacy GET behavior, and
Tahoe's absence of IOC 134 remain intact. Tahoe does not compile
`AirportVirtualIOCTL.cpp`. This is a historical source-surface fail-closed
boundary, not a Tahoe runtime, Apple terminal-status, carrier-layout,
sync-policy, firmware, CCA, event, or traffic parity claim. See
docs/reference/CR-513-legacy-awdl-sync-state-blind-success-quarantine-20260715.md.

## 2026-07-15 correction: legacy AWDL BSSID blind-success cache

The historical virtual-interface IOC 125 SET half previously copied only the
first six-byte MAC region into a local cache and returned success. Direct
current 25C56 nested-KEXT recovery shows a selector gate, nonzero-status
propagation, AWDL-protocol owner test, and dynamic tail dispatch; a failed
owner test returns raw `0xe082280e`. A separate internal handler requires a
non-null `0x10` request carrier and directly reaches an active
AWDL-peer-manager terminal that reads both six-byte carrier regions. The
local packed carrier is also `0x10` bytes, but byte-count equality is not ABI
or layout parity.

The active legacy SET half now returns `kIOReturnUnsupported` without reading
either argument. Its bidirectional route, first-MAC legacy GET cache behavior,
and Tahoe's absence of IOC 125 remain intact. Tahoe does not compile
`AirportVirtualIOCTL.cpp`. This is a historical source-surface fail-closed
boundary, not a Tahoe runtime, Apple terminal-status, carrier-layout,
...
## 2026-07-15 correction: legacy V1 BTCOEX_CONFIG non-null fixed-stub alignment

The historical V1 IOC 222 SET half in AirportSTAIOCTL.cpp previously copied a
non-null five-word carrier into a local BTCOEX readback cache and returned
success. Direct current 25C56 nested-KEXT recovery identifies the public
apple80211setBTCOEX_CONFIG body as an exact 11-byte fixed stub returning raw
0xe082280e. The public body is unread and contains no selector load, gate,
owner lookup, call, state, transport, event, or dispatch to a static handler
or terminal.
...
with direct public status alignment only; it does not claim null, Tahoe, GET,
carrier/ABI, BTCOEX policy, firmware, event, traffic, or runtime parity. See
docs/reference/CR-519-legacy-btcoex-config-non-null-fixed-stub-alignment-20260715.md.

## 2026-07-15 correction: legacy V1 BTCOEX_OPTIONS non-null fixed-stub alignment

The historical V1 IOC 235 SET half in AirportSTAIOCTL.cpp previously copied a
non-null options word into a local BTCOEX readback cache and returned success.
Direct current 25C56 nested-KEXT recovery identifies the public
apple80211setBTCOEX_OPTIONS body as an exact 11-byte fixed stub returning raw
0xe082280e. The public body is unread and contains no selector load, gate,
owner lookup, call, state, transport, event, or dispatch to a static handler
or terminal.
...
with direct public status alignment only; it does not claim null, Tahoe, GET,
carrier/ABI, BTCOEX policy, firmware, event, traffic, or runtime parity. See
docs/reference/CR-520-legacy-btcoex-options-non-null-fixed-stub-alignment-20260715.md.

## 2026-07-15 correction: legacy V1 BTCOEX_PROFILES non-null fixed-stub alignment

The historical V1 IOC 221 SET half in AirportSTAIOCTL.cpp previously freed,
allocated, and copied a non-null profiles carrier into a local BTCOEX
readback pointer before returning success. Direct current 25C56
nested-KEXT recovery identifies the public apple80211setBTCOEX_PROFILES body
as an exact 11-byte fixed stub returning raw 0xe082280e. The public body is
unread and contains no selector load, gate, owner lookup, call, state,
transport, event, or dispatch to a static handler or terminal.

The active legacy V1 setter now preserves its existing null guard and, only
for non-null input, leaves the carrier unread and returns that exact recovered
numeric status. The typed bidirectional V1 route, separate V1 GET replay, and
existing pointer teardown remain unchanged. Tahoe is already separate and
untouched: its Skywalk BTCOEX group returns its existing
kApple80211ClassOwnerAbsent status through the CR-479 direct-gate boundary,
and the Tahoe source phase compiles Skywalk rather than AirportSTAIOCTL.cpp.
This is non-null V1 false-success removal with direct public status alignment
