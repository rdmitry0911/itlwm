# CR-590 — Tahoe public SHORT_SLOT BSD SET fixed-stub alignment (2026-07-18)

## Scope

This layer changes only the canonical normal, non-null Tahoe BSD `SIOCSA80211`
route for `APPLE80211_IOC_SHORT_SLOT` (selector 33). It returns the exact
reference fixed status before inspecting the caller carrier. The independently
recovered GET status remains evidenced by CR-545 and is not otherwise changed.
...
`apple80211setSHORT_SLOT(IO80211SkywalkInterface*, void*)` is nlist 7276 at
VM `0xffffff80021c3a95` (file offset `0x020c3a95`). Its whole 11-byte body
ends at the adjacent `apple80211setMULTICAST_RATE` symbol at
`0xffffff80021c3aa0`:
...
`artifacts/skywalk-short-slot-set-public-fixed-stub-bootkc-25c56/raw.txt`.

## Change and boundary

The Tahoe BSD dispatcher now accepts only its existing normalized GET and SET
directions for IOC 33 and returns that fixed raw status after the existing
carrier-null guard. This is not a success acknowledgement and does not
construct, read, or activate the historical `apple80211_short_slot_data`
carrier declaration.
...
controller/card-specific ingress, short-slot timing state, firmware,
association, radio, traffic, and runtime selector invocation are not claimed
or changed.

## Verification

`scripts/test_tahoe_short_slot_set_contract.sh` verifies the independent SET
