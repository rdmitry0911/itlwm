# CR-519 — legacy V1 BTCOEX_CONFIG non-null fixed-stub alignment
...
APPLE80211_IOC_BTCOEX_CONFIG (IOC 222) in AirportSTAIOCTL.cpp. It retains the
typed packed apple80211_btc_config_data carrier, the existing bidirectional V1
...
Direct nested-LC_SYMTAB recovery identifies external section-1 nlist 7461,
__Z26apple80211setBTCOEX_CONFIGP23IO80211SkywalkInterfacePv, at half-open
VM/file ranges [0xffffff80021c5039, 0xffffff80021c5044) and
[0x20c5039, 0x20c5044). The next sorted external section symbol is nlist
7606, __Z28apple80211setAWDL_STATISTICSP23IO80211SkywalkInterfacePv, at
0xffffff80021c5044, so the recovered body is exactly 11 bytes:
...
The local packed carrier has five uint32_t words, but the public reference
stub is unread. No size or carrier-layout parity is inferred. The exact
identities, nlist/range boundary, raw bytes, body digest, and disassembly
command are retained in
docs/reference/artifacts/legacy-btcoex-config-public-fixed-stub-bootkc-current/raw.txt.

## Local divergence and correction

Before this correction, the historical V1 setter returned kIOReturnError for
a null carrier; for non-null input it memcpy'd the full carrier into the local
btcConfig readback cache and returned kIOReturnSuccess. The V1 dispatcher
admits GET and SET through its typed IOCTL macro. The paired V1 GET remains
separate existing behavior: after its own null guard, it copies btcConfig
back to its caller.
...
Tahoe remains separate and untouched. Skywalk groups BTCOEX_CONFIG with the
...
scripts/legacy_btcoex_config_non_null_fixed_stub_alignment_report.py --check
