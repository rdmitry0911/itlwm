# CR-623 — POWER_BUDGET nominal-identity прослойка (supersedes CR-492/CR-479)
...
`getPOWER_BUDGET`. It supersedes the no-producer quarantine recorded in
[CR-492-power-budget-no-producer-quarantine-20260715.md](CR-492-power-budget-no-producer-quarantine-20260715.md)
and the earlier [CR-479-power-budget-quarantine-20260714.md](CR-479-power-budget-quarantine-20260714.md).
...
Infra `getPOWER_BUDGET` wrapper `0x1000175d8` dispatches through virtual offset
`0x2f8` to Core `0x10010712c`. The recovered body reads `(Core + 0x48) + 0x4`,
stores it at caller `+0x4`, and returns zero (success). That Core scalar is a
TVPM (Broadcom thermal/voltage/power-management) budget index whose **own
default after `AppleBCMWLANCore::resetTVPMIndicies` is 100**. TVPM budget
...
Intel is not TVPM-throttling, so 100 (full budget) is Intel's actual nominal
...
`version = APPLE80211_VERSION`, `power_budget = 100`, and returns
...
`scripts/power_budget_quarantine_report.py --check` verifies the reference
...
slot and GET/SET dispatch, the still-fail-closed setter boundary, absence of a
