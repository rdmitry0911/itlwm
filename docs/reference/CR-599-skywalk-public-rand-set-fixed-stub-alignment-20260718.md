# CR-599 — Tahoe public RAND BSD SET fixed-stub alignment (2026-07-18)

## Scope

This layer changes only the canonical normal, non-null Tahoe BSD SIOCSA80211
route for APPLE80211_IOC_RAND (selector 45). It returns the exact reference
fixed status before inspecting the caller carrier. The independently recovered
GET status remains evidenced by CR-553 and is not otherwise changed.
...
apple80211setRAND(IO80211SkywalkInterface*, void*) is nlist 7028 at VM
0xffffff80021c3b19 (file offset 0x020c3b19). Its whole 11-byte body ends at
the adjacent apple80211setRSN_IE symbol at 0xffffff80021c3b24:
...
artifacts/skywalk-rand-set-public-fixed-stub-bootkc-25c56/raw.txt.

## Change and boundary

The Tahoe BSD dispatcher now accepts only its existing normalized GET and SET
directions for IOC 45 and returns that fixed raw status after the existing
carrier-null guard. This is not a success acknowledgement and does not
construct, read, or activate a local RAND carrier contract.
...
controller/card-specific ingress, RAND behavior, firmware, association, radio,
traffic, and runtime selector invocation are not claimed or changed. No V1,
Virtual, or card-specific RAND route is introduced.

## Verification

scripts/test_tahoe_rand_set_contract.sh verifies the independent SET raw record
