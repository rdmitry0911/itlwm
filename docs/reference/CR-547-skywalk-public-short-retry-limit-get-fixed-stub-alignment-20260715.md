# CR-547 — Skywalk public SHORT_RETRY_LIMIT GET fixed-stub alignment
...
APPLE80211_IOC_SHORT_RETRY_LIMIT (IOC 35) Tahoe BSD GET direction in
...
switch. SET behavior is separately aligned and documented by CR-592; this GET
evidence does not independently prove SET behavior.

No local SHORT_RETRY_LIMIT carrier contract is inferred from the direct
current public getter. This layer neither constructs nor consumes a carrier.

Historical V1 and Virtual IOCTL routes do not name IOC 35. The card-specific
route has no SHORT_RETRY_LIMIT entry and continues to exclude it through
...
apple80211getSHORT_RETRY_LIMIT at nlist 7721 in the half-open VM range
0xffffff80021bead1..0xffffff80021beadc, file range
0x20bead1..0x20beadc. It is an N_SECT|N_EXT record (type 0x0f, section 1,
...
preserved under
docs/reference/artifacts/skywalk-short-retry-limit-get-public-fixed-stub-bootkc-current/.

## Boundary and non-claims

This is an exact, narrow public GET status alignment. It does not claim
outer-null dispatch behavior, a SHORT_RETRY_LIMIT carrier contract,
...
parity. SET behavior is separately aligned and documented by CR-592; this GET
evidence does not independently prove SET behavior.
...
scripts/skywalk_public_short_retry_limit_get_fixed_stub_alignment_report.py
...
markers. CR-592 separately retains the independent SET raw record and SET
alignment contract.
