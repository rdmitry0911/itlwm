# CR-592 — Tahoe public SHORT_RETRY_LIMIT BSD SET fixed-stub alignment (2026-07-18)

## Scope

This layer changes only the canonical normal, non-null Tahoe BSD `SIOCSA80211`
route for `APPLE80211_IOC_SHORT_RETRY_LIMIT` (selector 35). It returns the
exact reference fixed status before inspecting the caller carrier. The
independently recovered GET status remains evidenced by CR-547 and is not
otherwise changed.
...
`apple80211setSHORT_RETRY_LIMIT(IO80211SkywalkInterface*, void*)` is nlist
7752 at VM `0xffffff80021c3aab` (file offset `0x020c3aab`). Its whole 11-byte
body ends at the adjacent `apple80211setLONG_RETRY_LIMIT` symbol at
`0xffffff80021c3ab6`:
...
`artifacts/skywalk-short-retry-limit-set-public-fixed-stub-bootkc-25c56/raw.txt`.

## Change and boundary

The Tahoe BSD dispatcher now accepts only its existing normalized GET and SET
directions for IOC 35 and returns that fixed raw status after the existing
carrier-null guard. This is not a success acknowledgement and does not
construct, read, or activate a local SHORT_RETRY_LIMIT carrier contract.
...
`scripts/test_tahoe_short_retry_limit_set_contract.sh` verifies the independent
