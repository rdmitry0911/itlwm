# CR-479: MWS association-protection bitmap false-success quarantine
...
`MWS_ASSOC_PROTECTION_BITMAP_WIFI_ENH`. It does not emulate Apple's Broadcom
MWS (coexistence) firmware policy on Intel hardware, and does not change a
neighboring MWS selector.
...
The Infra slot-[653] wrapper at `0x1000196cc` tail-dispatches to
`AppleBCMWLANCore::setMWS_ASSOC_PROTECTION_BITMAP_WIFI_ENH` at
`0x100141526`:
...
- a valid opaque carrier contributes nine effective dwords at `+0` through
  ...
  and
- the setter tail-dispatches virtual offset `+0x648`.

The recovered `__ZTV16AppleBCMWLANCore` starts at `0x1003a10d8`. The Core
object vptr address point is `+0x10`, so its virtual offset `+0x648` selects
raw entry `0x1003a1730`, whose image-local target is `0x100122fa6`,
`setWiFiAssocProtectionConfigBitmapWiFiEnh`, rather than a guessed local cache
consumer. That terminal allocates its transfer area and describes a 36-byte (`0x24`) `mws`
firmware-iovar packet with command `0x1018000` and subcommand `13`. It packs
nine low-16-bit bitmap fields, uses
`AppleBCMWLANCommander::sendIOVarSet` with
`handleMWSWiFiAssocProtConfigCoexBitmapsWiFiEnhAsyncCallback`, or
...
nine dwords into `cachedMwsAssocProtectionBitmap` and returned success. Scoped
local source inspection finds that cache only in its declaration, two
initializers, and this setter. There is no local `mws` iovar,
`setWiFiAssocProtectionConfigBitmapWiFiEnh` owner, matching callback,
