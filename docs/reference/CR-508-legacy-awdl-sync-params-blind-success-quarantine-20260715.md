# CR-508 — legacy AWDL sync-params blind-success shim quarantine

Date: 2026-07-15

## Scope

This correction covers only the historical `APPLE80211_IOC_AWDL_SYNC_PARAMS`
SET shim (IOC 116) in `AirportVirtualIOCTL.cpp`. It preserves the existing
packed local `apple80211_awdl_sync_params` carrier, the virtual-interface
dispatcher case, the paired existing legacy GET behavior, all neighboring AWDL
selectors, and Tahoe's existing absence of an IOC 116 route.

It does not alter the local carrier declaration or infer a missing field,
implement sync-parameter policy, change a selector number, add a route, change
...
`apple80211setAWDL_SYNC_PARAMS` wrapper at `0xffffff80021c42d5`. It is not a
fixed stub:

- it passes selector `0x74` (116) through the first owner vtable gate at
...
`0xffffff80021e433c` checks request length `0x18` and a non-null carrier
before an `IO80211AWDLPeerManager` safe-meta-cast-style cast/call through
metaclass `0xffffff80023e1460`.

The checked-in local packed carrier has exactly five `uint32_t` fields, hence
`0x14` bytes; the verified internal reference admission requires `0x18`. This
is an explicit local/reference carrier-size divergence. It does not license a
guess at a sixth field, a header change, an equivalence claim, or private
...
null-input behavior, valid-input status, sync-parameter policy, state
mutation, event, or firmware semantics. It also must not be read as a claim
that the reference selector never validates the carrier. `0xe082280e` is
...
`docs/reference/artifacts/awdl-sync-params-public-wrapper-bootkc-current/raw.txt`.
...
The legacy virtual dispatcher reaches its admitted SET half for IOC 116, while
...
`scripts/legacy_awdl_sync_params_blind_success_quarantine_report.py --check`
verifies the raw-artifact identity/manifest, public conditional-wrapper
anchors, separate internal admission caveat, and preserved `0x14` versus
`0x18` size-divergence record; the retained selector, carrier, dispatcher,
